...
async function handleVote(interaction, choice, voteId) {
  ...
  // Chercher par ID ou par messageId si pas d'ID dans le bouton
  const vote = voteId
...
function buildEmbed(subject, mode, smash, pass) {
  ...
  const pct   = total > 0 ? Math.round((smash / total) * 100) : 0;
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: '💚 Smash', value: `**${smash}** vote${smash > 1 ? 's' : ''}`, inline: true },
      { name: '💔 Pass',  value: `**${pass}** vote${pass > 1 ? 's' : ''}`,   inline: true },
      { name: `Popularité — **${pct}% l'ont smashé** sur ${total} vote${total > 1 ? 's' : ''}`,
