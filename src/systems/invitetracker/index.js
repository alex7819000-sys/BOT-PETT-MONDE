...
async function handleMemberJoin(member) {
  ...
  if (!usedInvite?.inviter) {
    // ── Cas invite vanity (discord.gg/serveur) ──────────────────────────
    // Dans ce cas aucune invite standard n'a bougé
    // On essaie de vérifier l'invite vanity
    try {
      const vanity = await guild.fetchVanityData().catch(() => null);
      if (vanity?.uses > (inviteCache.get(gid + ':vanity') || 0)) {
        inviteCache.set(gid + ':vanity', vanity.uses);
        // Personne à créditer pour l'invite vanity — on notifie juste le join
        ...
        if (notifChannelId) {
          ...
          if (channel) await channel.send(`👋 Bienvenue <@${member.id}> ! On est maintenant **${member.guild.memberCount}** 🎉`);
        }
      }
    } catch (_) {}
    return;
  ...
  // ── Ne jamais créditer un bot comme inviteur ─────────────────────────
  if (usedInvite.inviter.bot) {
    logger.warn('InviteTracker', `Inviteur est un bot (${inviterId}) — ignoré`);
    return;
  }

  // ── Ne pas se créditer soi-même ──────────────────────────────────────
