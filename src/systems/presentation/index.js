...
async function sendWelcomeDM(member, client) {
  try {
    ...
    // ── Menu couleur dès le DM de bienvenue ───────────────────────────────
    ...
    if (colorRoles.length > 0) {
      ...
      const colorSelect = new StringSelectMenuBuilder()
        .setCustomId(`color_role:select:${guildId}`)
        .setPlaceholder('🎨 Choisis ta couleur de pseudo...')
        .addOptions(options);
      components.push(new ActionRowBuilder().addComponents(colorSelect));
    }
...
async function finaliserPresentation(user, guildId, pres, client, interaction) {
  ...
  // ── Proposer le choix de couleur en DM ───────────────────────────────────
  try {
    const colorRoles = config?.colorRoleIds || [];
    if (colorRoles.length > 0) {
      const options = colorRoles.slice(0, 25).map(cr =>
      ...
      const selectMenu = new StringSelectMenuBuilder()
        ...
        .setPlaceholder('🎨 Choisis ta couleur de pseudo...')
        .addOptions(options);
      const colorRow = new ActionRowBuilder().addComponents(selectMenu);
      const colorEmbed = new EmbedBuilder()
        .setColor(0x57F287)
        .setTitle('🎉 Présentation envoyée ! Choisis ta couleur')
        .setDescription(
          '✅ Ta présentation est maintenant publiée sur le serveur !\n\n' +
          '🎨 **Choisis la couleur de ton pseudo** grâce au menu ci-dessous.\n' +
          '**1 couleur à la fois** — si tu en choisis une nouvelle, l\'ancienne est retirée automatiquement.\n\n' +
          '> 💡 Tu peux changer de couleur à tout moment avec `/couleur` sur le serveur.'
        );
      const dmChannel = await user.createDM();
      await dmChannel.send({ embeds: [colorEmbed], components: [colorRow] });
    }
  } catch (_) {
    // DM désactivés ou pas de rôles couleur — on ignore silencieusement
  }
}
