...
// Flow : guildMemberAdd → DM welcome → bouton "Me présenter"
//   → dans le serveur: /presentation reprendre → bouton → modal étape 1
//   → modal étape 2 → ... → modal étape 5 → post forum + rôle
'use strict';
...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIG DES 5 MODALS (max 5 champs par modal, texte courts)
// ─────────────────────────────────────────────────────────────────────────────
const MODALS = [
  {
    id: 'present_modal_1',
...
// ─────────────────────────────────────────────────────────────────────────────
// EMBED DE PROGRESSION (message éditable dans le salon)
// ─────────────────────────────────────────────────────────────────────────────
...
async function sendWelcomeDM(member, client) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: 'Utilise /presentation reprendre dans le serveur pour commencer !' });
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. Afficher le panneau de progression + bouton "Remplir l'étape"
//    (dans le serveur, réponse éphémère)
// ─────────────────────────────────────────────────────────────────────────────
async function showProgressPanel(interaction, client, pres) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`present:open_modal:${step}:${interaction.guild.id}`)
      .setLabel(`${MODALS[step - 1].emoji} Remplir l'étape ${step} — ${MODALS[step - 1].titre}`)
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(`present:skip_modal:${step}:${interaction.guild.id}`)
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. Ouvrir le modal (bouton → modal popup)
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleModalSubmit(interaction, client) {
  ...
  // Sinon → afficher le panel de progression mis à jour
  pres.step = nextStep;
  const progressEmbed = buildProgressEmbed(interaction.user, nextStep);

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`present:open_modal:${nextStep}:${guildId}`)
      .setLabel(`${MODALS[nextStep - 1].emoji} Remplir l'étape ${nextStep} — ${MODALS[nextStep - 1].titre}`)
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(`present:skip_modal:${nextStep}:${guildId}`)
      .setLabel('Passer cette étape →')
      .setStyle(ButtonStyle.Secondary),
  );

  await interaction.update({ embeds: [progressEmbed], components: [row] }).catch(async () => {
    // Si interaction expirée, répondre normalement
    await interaction.reply({ embeds: [progressEmbed], components: [row], ephemeral: true }).catch(() => {});
  });
}

// ─────────────────────────────────────────────────────────────────────────────
// 5. Passer une étape (bouton "Passer cette étape →")
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. Finaliser : créer le post forum + donner le rôle
// ─────────────────────────────────────────────────────────────────────────────
async function finaliserPresentation(user, guildId, pres, client, interaction) {
  ...
  if (interaction && !interaction.replied && !interaction.deferred) {
    await interaction.reply({ embeds: [doneEmbed], components: [], ephemeral: true }).catch(() => {});
  } else if (interaction) {
...
module.exports = {
  ...
  handleVoirCommand,
  // compat: ancienne API (au cas où buttons.js appelle startQuestionnaire)
