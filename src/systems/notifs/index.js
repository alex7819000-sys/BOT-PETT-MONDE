...
async function checkYouTube(client, guildId) {
  ...
  try {
    ...
    await channel.send({ content: `${config?.youtubeRoleId ? '<@&' + config.youtubeRoleId + '> ' : ''}🎬 Nouvelle vidéo !`, embeds: [embed], components: [row] });
...
async function checkTwitch(client, guildId) {
  ...
  try {
    ...
    if (isLive && !config.twitchIsLive) {
      ...
      await channel.send({ content: `${config?.twitchRoleId ? '<@&' + config.twitchRoleId + '> ' : ''}🔴 Le live a commencé !`, embeds: [embed], components: [row] });
