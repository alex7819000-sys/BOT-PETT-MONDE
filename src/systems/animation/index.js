...
async function postAnnonce(interaction, client) {
  ...
  // Construire la liste des rôles autorisés à mentionner
  const allowedRoles = [];
  if (ping?.id)                    allowedRoles.push(ping.id);
  else if (config?.announcePingRoleId) allowedRoles.push(config.announcePingRoleId);
  if (config?.memberRolePingId)    allowedRoles.push(config.memberRolePingId);

  await salon.send({
    content: pingText,
    embeds: [embed],
    allowedMentions: { roles: allowedRoles, everyone: false },
  });
