...
async function addXP(userId, guildId, amount) {
  try {
    ...
    let xpGain = amount;
    if (user.xpBoostUntil && user.xpBoostUntil > new Date()) xpGain *= 2;      // x2 guilde dominante
    if (user.podiumBoostUntil && user.podiumBoostUntil > new Date()) xpGain = Math.round(xpGain * 1.5); // +50% podium
