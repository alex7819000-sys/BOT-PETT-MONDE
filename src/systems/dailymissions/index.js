...
async function postDailyMissionsBoard(client, guildId) {
  ...
  const embed = new EmbedBuilder()
    .setColor(0x5865F2)
    .setTitle(`📅 Missions du jour — ${dateCapital}`)
    .setDescription(
      lines.join('\n\n') +
      '\n\n' +
      '> 💡 Tape `/niveau` pour voir ta progression en temps réel !'
    )
    .addFields(
      { name: '🎁 Bonus si 3/3 complétées', value: '+100 XP · +200 💎 · **+50% XP pendant 24h** 🔥', inline: false },
      { name: '⏰ Reset', value: 'Chaque nuit à minuit', inline: true },
      { name: '📊 Voir ta progression', value: '`/niveau` ou `/missions`', inline: true },
    )
    .setTimestamp()
    .setFooter({ text: 'King Bot · Missions quotidiennes' });

  // Mentions
  const pingMembre  = config?.membreRoleId    ? `<@&${config.membreRoleId}>`    : '';
  const pingAnnonce = config?.announceRoleId  ? `<@&${config.announceRoleId}>` : '';
  const mention     = [pingMembre, pingAnnonce].filter(Boolean).join(' ');
  const content     = mention
    ? `${mention}\n📅 **NOUVELLES MISSIONS DU JOUR DISPONIBLES !**`
    : '📅 **NOUVELLES MISSIONS DU JOUR DISPONIBLES !**';

  // 1. Post dans le salon missions (si configuré)
  if (config?.missionsChannelId) {
    const missionsCh = guild.channels.cache.get(config.missionsChannelId);
    if (missionsCh) {
      await missionsCh.send({ content, embeds: [embed] }).catch(() => {});
    }
  }

  // 2. Post dans le salon annonce (si différent du salon missions)
  const announceCh = config?.announceChannelId
    ? guild.channels.cache.get(config.announceChannelId)
    : null;

  if (announceCh && announceCh.id !== config?.missionsChannelId) {
    // Embed plus court pour l'annonce, renvoie vers /niveau
    const announceEmbed = new EmbedBuilder()
      .setColor(0x5865F2)
      .setTitle(`📅 Nouvelles missions du jour — ${dateCapital}`)
      .setDescription(
        lines.join('\n\n') +
        '\n\n' +
        '🎁 **Bonus 3/3 :** +100 XP · +200 💎 · +50% XP 24h 🔥\n' +
        '> Utilise `/niveau` pour voir ta progression !'
      )
      .setTimestamp()
      .setFooter({ text: 'Reset à minuit · King Bot' });

    await announceCh.send({ content, embeds: [announceEmbed] }).catch(() => {});
  }

  logger.info('DailyMissions', 'Board missions posté');
}
