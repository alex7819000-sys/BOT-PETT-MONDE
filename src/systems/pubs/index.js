...
async function handleModalPubTicket(interaction) {
  ...
  const ticketChannel = await guild.channels.create({
    name: `📢-pub-${interaction.user.username.slice(0, 12).toLowerCase()}`,
    type: ChannelType.GuildText,
    parent: config?.pubTicketCategoryId || config?.staffCategoryId || null,
...
async function handlePubValider(interaction) {
  ...
  // Récupérer le message avec la fiche
  const ficheEmbed = interaction.message.embeds[0];
  if (!ficheEmbed) return interaction.reply({ content: '❌ Fiche introuvable.', ephemeral: true });

  // Publier dans le salon pub
  ...
  if (pubChannel) {
    const publishEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: `Publié par ${interaction.guild.name}` })
      .setTimestamp();
    ...
    await pubChannel.send({ embeds: [publishEmbed] });
  }
  ...
  await interaction.reply({ content: `✅ Publication validée${pubChannel ? ` et postée dans <#${pubChannel.id}>` : ''} !` });

  // Fermer le ticket après 10s
  setTimeout(async () => {
    try { await interaction.guild.channels.cache.get(channelId)?.delete('Pub validée'); } catch (_) {}
  }, 10000);
}
...
async function handlePubRefuser(interaction) {
  ...
  await interaction.message.edit({ components: [] });
  await interaction.reply({ content: '❌ Publication refusée. DM envoyé au demandeur.' });

  setTimeout(async () => {
    try { await interaction.guild.channels.cache.get(channelId)?.delete('Pub refusée'); } catch (_) {}
  }, 10000);
}

module.exports = Object.assign(module.exports, {
  handlePubDemande,
  handleModalPubTicket,
  handlePubValider,
  handlePubRefuser,
});
