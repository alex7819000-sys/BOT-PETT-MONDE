...
async function handleWarTrigger(message) {
  ...
  // Équilibrage dynamique : les perdants ont un multiplicateur bonus
  {
    ...
    await addXP(uid, gid, XP_PER_POINT * multiplier);
    ...
    if (prev.leader !== newLeader && newTotal > 20) {
      ...
      const pingRole = cfg3?.announceRoleId ? `<@&${cfg3.announceRoleId}>` : '';
...
async function runWarCeremony(client, guildId) {
  ...
  await channel.send({ content: `${cfg3?.announceRoleId ? '<@&' + cfg3.announceRoleId + '> ' : ''}⚔️ Résultats de la guerre !`, embeds: [embed] });
