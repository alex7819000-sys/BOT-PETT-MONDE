...
async function handleQuizAnswer(interaction, encodedAnswer) {
  ...
  if (correct) {
    ...
    const dailyBonus = await xpSys.checkDailyGameBonus(uid, gid, interaction.channelId, interaction.guild);
    if (dailyBonus) bonusLine = `\n🎯 **+${dailyBonus} XP bonus** — première action du jour dans Quiz Anime !`;
    ...
    await interaction.followUp({
      content: [
        `${rankLabel} **+${diff.xp} XP** (${diff.label})${speedBonus > 0 ? ` + **+${speedBonus} XP** bonus vitesse` : ''} = **+${totalXp} XP** total${bonusLine}`,
