...
async function startVote(client, guildId) {
  ...
  const msg = await channel.send({ content: `${config?.announceRoleId ? '<@&' + config.announceRoleId + '> ' : ''}🚨 Le vote pour le Singe commence !`, embeds: [embed], components: [row] });
...
async function runSingeCeremony(client, guildId) {
  ...
  if (channel) {
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    await channel.send({ content: `${config?.announceRoleId ? '<@&' + config.announceRoleId + '> ' : ''}${EMOJIS.SINGE} Le singe est élu !`, embeds: [embed] });
  }
