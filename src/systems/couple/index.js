...
async function startVote(client, guildId) {
  ...
  const msg = await channel.send({ content: `${config?.announceRoleId ? '<@&' + config.announceRoleId + '> ' : ''}💑 Votez pour le meilleur couple !`, embeds: [embed], components: [row] });
...
async function runCoupleCeremony(client, guildId) {
  ...
  await channel.send({ content: `${config?.announceRoleId ? '<@&' + config.announceRoleId + '> ' : ''}${EMOJIS.COUPLE} Annonce du meilleur couple !`, embeds: [embed] });
