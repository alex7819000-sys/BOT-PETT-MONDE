...
async function startSchedulers(client) {
  ...
  cron.schedule('30 20 * * *', async () => {
    try {
      const { runRoiDuJour } = require('../systems/animation');
      await runRoiDuJour(client, guildId);
    } catch (e) { logger.error('Cron', 'Roi du jour', e); }
  ...
  cron.schedule('0 12 * * *', async () => {
    ...
    // Vérification inactivité staff quotidienne
    try {
      const { checkInactivity } = require('../systems/kingstaff');
      await checkInactivity(client, guildId);
    } catch (e) { logger.error('Cron', 'Staff inactivity check', e); }

    // Mise à jour live board staff
    try {
      const config = await Config.findOne({ guildId });
      if (config?.staffLiveBoardMessageId && config?.staffClassementChannelId) {
        const guild = client.guilds.cache.get(guildId);
        if (guild) {
          const { updateStaffLiveBoard } = require('../systems/kingstaff');
          await updateStaffLiveBoard(guild, config);
        }
      }
    } catch (e) { logger.error('Cron', 'Staff live board', e); }
  }, { timezone: 'Europe/Paris' });
  ...
  cron.schedule('* * * * *', async () => {
    ...
    const { runKingStaffCeremony } = require('../systems/kingstaff');
    ...
    await runKingStaffCeremony(client, guildId);
  });
  ...
  // ── Missions quotidiennes — reset minuit + post board 8h ──────────────
  cron.schedule('0 0 * * *', async () => {
    try {
      const { resetDailyMissions } = require('../systems/dailymissions');
      await resetDailyMissions(guildId);
      logger.info('Cron', 'Missions quotidiennes reset');
    } catch (e) { logger.error('Cron', 'Daily missions reset', e); }
  }, { timezone: 'Europe/Paris' });

  cron.schedule('0 8 * * *', async () => {
    try {
      const { postDailyMissionsBoard } = require('../systems/dailymissions');
      await postDailyMissionsBoard(client, guildId);
      logger.info('Cron', 'Missions quotidiennes postées');
    } catch (e) { logger.error('Cron', 'Daily missions board', e); }
  }, { timezone: 'Europe/Paris' });
