...
async function startSchedulers(client) {
  ...
  // ── Défis quotidiens — 10h chaque matin ──────────────────────────────
  cron.schedule('0 10 * * *', async () => {
    try {
      const guild = client.guilds.cache.get(guildId);
      if (!guild) return;
      const { lancerDefiQuotidien } = require('../systems/defis');
      await lancerDefiQuotidien(guild, client);
    } catch (e) { logger.error('Cron', 'Défi quotidien', e); }
  }, { timezone: 'Europe/Paris' });

  // ── King Challenge — jeudi 19h (avant reset dimanche) ────────────────
  cron.schedule('0 19 * * 4', async () => {
    try {
      const guild = client.guilds.cache.get(guildId);
      if (!guild) return;
      const { lancerKingChallenge } = require('../systems/defis');
      await lancerKingChallenge(guild, client);
    } catch (e) { logger.error('Cron', 'King Challenge', e); }
  }, { timezone: 'Europe/Paris' });
