...
async function handleFaceRevealAuto(message) {
  // Si c'est un fil → laisser les gens commenter librement
  if (message.channel.isThread()) return false;
  ...
  if (!hasImage) {
    // Texte pur → supprimer + guider vers les threads
    await message.delete().catch(() => {});
    const warn = await message.channel.send({
      content: `<@${message.author.id}> ❌ Poste une **image** ici ! Pour commenter, utilise les fils de discussion sous chaque photo 🧵`,
    });
    setTimeout(() => warn.delete().catch(() => {}), 6000);
    return true;
  }

  // Image valide → soumettre au système SOP + créer thread
  const attachment = message.attachments.first();
  if (attachment) {
    const { submitCommunity } = require('../systems/smash');
    const pseudo = message.member?.displayName || message.author.username;
    await submitCommunity(message.client, message.guild.id, 'face-reveal', {
      name: pseudo,
      imageUrl: attachment.url,
      extra: '🤫',
      submittedBy: message.author.id,
    });
    // Supprimer le message original (le bot reposte via SOP)
    await message.delete().catch(() => {});
  }
  return true;
}
