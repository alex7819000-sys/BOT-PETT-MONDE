...
async function handleMediaChannel(message) {
  // ── Fils de discussion : tout est autorisé (texte, images, vidéos) ────
  ...
  // • Il contient un fichier joint (image/vidéo/gif) AVEC ou SANS texte
  // • Il contient un lien vers une plateforme média
  ...
  const hasMediaLink =
    ...
    /\.(jpg|jpeg|png|gif|webp|mp4|mov|webm|avi|mkv|gifv)(\?.*)?$/i.test(txt) ||
    /https?:\/\/(www\.)?(youtube\.com|youtu\.be|tiktok\.com|twitter\.com|x\.com|instagram\.com|imgur\.com|tenor\.com|giphy\.com|clips\.twitch\.tv|streamable\.com|medal\.tv|vimeo\.com)/i.test(txt);

  const hasMedia = hasAttachment || hasMediaLink;

  if (!hasMedia) {
    // Texte pur sans média → supprimer + guide 6s
    await message.delete().catch(() => {});
    const warn = await message.channel.send({
      content: `<@${message.author.id}> ❌ Ce salon est réservé aux **photos et vidéos**. Commente dans le fil sous chaque média 🧵`,
    });
...
async function handleFaceRevealAuto(message) {
  ...
  if (!hasImage) {
    await message.delete().catch(() => {});
    const warn = await message.channel.send({
      content: `<@${message.author.id}> ❌ Poste une **image ou vidéo** ici ! Pour commenter, utilise les fils 🧵`,
    });
    setTimeout(() => warn.delete().catch(() => {}), 6000);
    return true;
  }
  ...
  // On NE supprime PAS le message original — l'image reste visible
  // Les boutons SOP sont en réponse directe
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('sop:smash').setLabel('💚 Smash').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('sop:pass').setLabel('💔 Pass').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('sop:stats').setLabel('📊 Stats').setStyle(ButtonStyle.Secondary),
  );

  const reply = await message.reply({
    content: `💅 **Face Reveal de ${pseudo}** — Smash or Pass ?`,
    components: [row],
  });
  ...
  await Vote.create({
    ...
    mode: 'face-reveal',
    subject: { name: pseudo, imageUrl: attachment?.url || '', extra: '🤫', submittedBy: message.author.id },
    messageId: reply.id,
    channelId: message.channel.id,
  });

  // Thread pour commenter
  try {
    const thread = await message.startThread({
    ...
    await thread.send(`💬 Commente le face reveal de **${pseudo}** ici !`);
  } catch (_) {}
