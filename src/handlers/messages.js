...
const { detectBump, trackHumanMessage } = require('../systems/bump');
...
async function handleMessage(message, client) {
  ...
  // Tracker le dernier humain par salon (pour attribution bump Disboard)
  trackHumanMessage(message);
  ...
  // ── Bonus première commande Mudae du jour ────────────────────────────────
  if (config?.mudaeChannelId && message.channel.id === config.mudaeChannelId) {
    const MUDAE_CMDS = /^\$(w|wa|wg|hg|dk|mk|rolls|roll|k|ka|ha|ma|hk|mg|ha|ua|uw|rk|wai|mar)\b/i;
    if (MUDAE_CMDS.test(message.content.trim())) {
      const { checkDailyGameBonus } = require('../systems/xp');
      const bonus = await checkDailyGameBonus(uid, gid, message.channel.id, message.guild);
      if (bonus) {
        const notif = await message.channel.send(`🎯 <@${uid}> **+${bonus} XP bonus** — première commande Mudae du jour ! 🎌`).catch(() => null);
  ...
  // 9. Surveillance Ni Oui Ni Non
  try {
    const { checkNiOuiNiNon } = require('../systems/defis');
    await checkNiOuiNiNon(message);
  } catch (_) {}
}
