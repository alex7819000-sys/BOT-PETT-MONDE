...
Actif entre **16h et 22h** automatiquement.`,
  ...
  if (sub === 'animation') {
    ...
    const roleKeys    = ['membreRoleId', 'roiDuJourRoleId', 'announcePingRoleId', 'announceRoleId', 'boostPingRoleId', 'boostRoleId', 'dailyBonusRoleId'];
  ...
  // ── Ping Roles — poster le message boutons ────────────────────────────────
  if (sub === 'pingroles') {
    const { checkPermission } = require('../../systems/hierarchy');
    const ok = await checkPermission(interaction, 'setup_config');
    if (!ok) return;
    ...
    const { postPingRolesMessage } = require('../../systems/pingroles');
    const posted = await postPingRolesMessage(client, gid, salon.id);

    if (!posted) {
      return interaction.editReply({
        content: `❌ Aucun rôle ping n'est configuré.\nConfigure d'abord les rôles via \`/setup role\` :\n• 📣 Rôle annonces\n• 🎬 Rôle YouTube\n• 🟣 Rôle Twitch\n• 🎁 Rôle giveaways\n• ⚔️ Rôle défis\n• 🔵 Rôle bump`,
      });
    }

    return interaction.editReply({
      content: `✅ Message des rôles ping posté dans <#${salon.id}> !\nLes membres peuvent maintenant cliquer pour s'abonner aux notifications.`,
    });
  }
