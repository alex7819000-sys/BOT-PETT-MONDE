...
async function handleButton(interaction, client) {
  ...
  try {
    switch (ns) {
      ...
      case 'present': {
        const { startQuestionnaire, skipEtape } = require('../systems/presentation');
        // Bouton "Me présenter" dans le DM de bienvenue
        if (action === 'start') {
          const guildId = rest[0];
          return startQuestionnaire(interaction, client, guildId);
        }
        // Bouton "dm_start" depuis /presentation reprendre
        if (action === 'dm_start') {
          const guildId = rest[0];
          return startQuestionnaire(interaction, client, guildId);
        }
        ...
        if (action === 'skip_etape') {
          return skipEtape(interaction, client);
        }
        // Bouton modifier depuis /presentation reprendre
        if (action === 'modifier_start') {
          const { handleModifierCommand } = require('../systems/presentation');
          return handleModifierCommand(interaction, client);
        }
        break;
      }
      case 'profile': {
