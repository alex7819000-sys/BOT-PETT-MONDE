...
async function buildStatusPage(client) {
  ...
  const memberOf = (userId) => {
    ...
    return m ? esc(m.displayName || m.user.username) : `Membre #${esc(String(userId).slice(-4))}`;
  };

  const kingOfDayName = singeElection?.winners?.[0] ? memberOf(singeElection.winners[0]) : null;

  const topRows = topUsers.length
    ? topUsers.map((u, i) => `
        <tr>
          <td class="rk">#${i + 1}</td>
          <td>${esc(u.username) || memberOf(u.userId)}</td>
          <td class="num">Niv. ${u.level ?? 0}</td>
          <td class="num gold">${(u.totalXp || 0).toLocaleString('fr-FR')} XP</td>
        </tr>`).join('')
    : `<tr><td colspan="4" class="empty">Pas encore de données XP.</td></tr>`;

  const botStatus = client.ws?.status === 0 ? 'En ligne' : 'Connexion…';
  const memberCount = guild ? guild.memberCount.toLocaleString('fr-FR') : '—';
  const guildName = guild ? esc(guild.name) : 'Serveur non configuré';
  const guildIcon = guild?.iconURL?.({ size: 128 }) || null;
  const countingCurrent = cfg?.countingCurrent ?? 0;
  const countingBest = cfg?.countingBestStreak ?? 0;
...
<body>
  <div class="wrap">
    ...
    <div class="king-card"><span class="crown">⭐</span><div><div class="name">${memberOf(kingStaff.userId)}</div><div class="tag">${kingStaff.weekScore} points cette semaine</div></div></div>
    ` : ''}
...
module.exports = { buildStatusPage };
