...
const CURRENT_VERSION = 'v104';

const CHANGELOG = {
  v104: {
    title: 'Rattrapage des rôles de niveau',
    added: [
      'Nouvelle commande `/setup syncroles` : réattribue les rôles de niveau à **tous les membres du serveur** d\'un coup, selon leur niveau actuel — utile pour les membres qui ont déjà gagné de l\'XP mais n\'ont jamais reçu leur rôle (rôle retiré par erreur, palier ajouté après coup, etc.), sans attendre qu\'ils passent un nouveau niveau',
    ],
  },

  v103: {
    title: 'King of Hera — trophée hebdomadaire',
    added: [
      'Nouveau rôle unique "King of Hera" : chaque dimanche à minuit, il est automatiquement donné au membre avec le plus d\'XP gagné dans la semaine, et retiré à l\'ancien détenteur — comme un vrai trophée qui change de mains. Si personne n\'a gagné d\'XP cette semaine, le rôle n\'est attribué à personne',
      'Se configure avec `/setup role` (type "👑 King of Hera")',
    ],
    changed: [
      'Remplace le système de paliers hebdo (plusieurs rôles selon des seuils d\'XP/semaine, désormais supprimés côté configuration) par ce rôle unique — plus simple, un seul gagnant par semaine plutôt que plusieurs paliers',
    ],
  },

  v102: {
    title: 'Le chat IA accueille les nouveaux membres',
    added: [
      'En plus du ping Chat Revive, le bot envoie maintenant un petit message spontané au nouveau membre (généré par le chat IA) pour engager la conversation et le mettre à l\'aise — genre "Salut, comment tu vas ?", avec sa propre personnalité plutôt qu\'un message d\'accueil robotique',
      'Léger délai (3s) après le ping pour ne pas arriver en même temps, et la conversation garde le fil : si le nouveau membre répond dans ce salon, le chat IA se souvient de son propre message',
      'Nécessite que le chat IA soit configuré (clé Gemini) — sinon ce message bonus est simplement ignoré, sans erreur. Consomme le même quota journalier que le reste du chat IA',
    ],
  },

  v101: {
    title: 'Chat Revive : ping des membres à l\'arrivée',
    added: [
      'Nouvelle fonctionnalité "Chat Revive" : à chaque nouveau membre qui rejoint le serveur, le bot ping un rôle dédié (et mentionne directement la personne) dans un salon choisi — pour inciter la communauté à venir l\'accueillir',
      'Tout se configure en une seule commande : `/setup chatrevive salon:#xxx role:@xxx minutes:90` (chaque option est indépendante, tu peux n\'en changer qu\'une seule à la fois). Taper la commande sans rien remplir affiche la configuration actuelle. ⚠️ Le champ `minutes` est pour l\'instant juste enregistré en base, sans effet réel — réservé à une future fonctionnalité de ping en cas d\'inactivité du chat, pas encore développée',
      'Le rôle et le salon restent aussi réglables séparément via `/setup role` (type "💬 Ping Chat Revive") et `/setup salon` (type "💬 Chat Revive"), pour ceux qui préfèrent',
    ],
    fixed: [
      'Protection anti-spam ajoutée : si plusieurs membres rejoignent d\'affilée, un seul ping part maximum toutes les 5 minutes, pour ne pas spammer le salon',
    ],
  },

  v100: {
    title: 'Créateur d\'embeds réparé',
    fixed: [
      'Un bouton "Lien" sans URL faisait planter l\'envoi de tout le message — corrigé',
      'Les boutons de couleur (bleu/gris/vert/rouge) étaient décoratifs, ne faisaient rien au clic — ils déclenchent maintenant de vraies actions du bot (ticket, signalement, candidature staff, partenariat)',
    ],
  },

  v99: {
    title: 'Système "sass" retiré',
    removed: [
      'L\'ancien système de réponses toutes faites (réagir aux insultes, au "brainrot"...) est retiré — le vrai chat IA a pris le relais',
    ],
  },

  v98: {
    title: 'Phrases coupées corrigées',
    fixed: [
      'Le chat IA coupait parfois ses réponses en plein milieu d\'un mot — corrigé, il termine maintenant proprement sa phrase (ou ajoute "…" si vraiment nécessaire)',
    ],
  },

  v97: {
    title: 'Chat IA encore plus patient',
    changed: [
      'Le chat IA attend maintenant plus longtemps et réessaie plus de fois avant d\'abandonner face à une surcharge Google — plus rare de voir un échec, quitte à répondre un peu plus lentement de temps en temps',
    ],
  },

  v96: {
    title: 'Chat IA plus robuste',
    fixed: [
      'Les échecs occasionnels du chat IA venaient d\'une surcharge des serveurs Google, pas d\'un bug ici — ajout de nouvelles tentatives automatiques avant d\'abandonner, ça devrait beaucoup réduire les "j\'ai eu un souci"',
    ],
  },
  ...
  v95: {
    title: 'Feur retiré',
    removed: [
      'Système "feur" (easter egg quoi→feur, comment→tateur) entièrement retiré, sur demande',
    ],
  },
