...
async function handleLevelUp(message, newLevel, user) {
  try {
    ...
    // ── Rôle principal actuel (toujours affiché) + détection "vient d'être débloqué" ──
    // roleActuel = le palier le plus haut déjà atteint au niveau newLevel — affiché à
    // CHAQUE level up, pas seulement quand un nouveau palier vient d'être franchi.
    // roleDebloque = sous-cas de roleActuel, uniquement si ce palier vient tout juste
    // d'être franchi depuis le dernier niveau connu (capture aussi les sauts de niveau,
    // ex: niveau 9 → 12 avec un palier à 10 configuré) — sert pour le libellé et le tip.
    let roleActuel = null;
    let roleDebloque = null;
    if (cfg?.levelRoles?.length) {
      const oldLevel = user.level || 0;
      const sorted   = [...cfg.levelRoles].sort((a, b) => b.level - a.level);
      roleActuel     = sorted.find(lr => lr.level <= newLevel) || null;
    ...
    if (roleActuel) {
      rolesLines.push(
        roleDebloque
          ? `🎖️ Nouveau rôle débloqué → <@&${roleActuel.roleId}>`
          : `🎖️ Rôle de niveau actuel → <@&${roleActuel.roleId}>`
      );
    }
    ...
    .addFields(
      {
        ...
        value: [
          `> 💬 **Messages** — ${cfg?.xpPerMessage || 15} XP par message (cooldown ${cfg?.xpCooldown || 60}s entre 2 gains)`,
          '> 🎙️ **Vocal** — 25 XP toutes les 5 minutes passées en vocal, + bonus de fidélité jusqu\'à +100% après 10h d\'affilée',
          '> 🚀 **Bump du serveur** (Disboard)',
          '> 🤫 **Poster une confession**',
          '> ✅ **Compléter tes missions/quêtes** quotidiennes',
          '> 💜 **Booster le serveur** (bonus unique)',
        ].join('\n'),
      },
      {
      ...
      {
        name: '📊 Pour voir ta progression',
        value: '> Le prochain level up t\'affichera ton niveau, ton classement et ta barre de progression — pas besoin de commande, ça s\'affiche automatiquement à chaque palier.',
      },
    )
