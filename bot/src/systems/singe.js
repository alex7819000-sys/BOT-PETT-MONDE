// src/systems/singe.js — Singe du serveur (nomination, vote, cérémonie, règle du mot)
'use strict';
...
async function nominate(interaction, target) {
  ...
  const existing = await Nomination.findOne({ guildId: gid, type: 'singe', nominatorId: uid, week, year });
  if (existing) return interaction.editReply({ content: '❌ Tu as déjà nominé quelqu\'un cette semaine.' });
  ...
  try {
    ...
    return interaction.editReply({ content: '❌ Erreur lors de la nomination.' });
  }
...
async function startVote(client, guildId) {
  ...
  const pingContent = config?.announceRoleId ? `<@&${config.announceRoleId}> 🚨 Le vote pour le Singe commence !` : '🚨 Le vote pour le Singe commence !';
  const msg = await channel.send({ content: pingContent, embeds: [embed], components: [row] });

  await Election.create({ guildId, type: 'singe', phase: 'vote', candidates, messageId: msg.id, channelId: channel.id, week, year });
...
async function handleVote(interaction, targetId) {
  ...
  return interaction.followUp({ content: `✅ Vote enregistré pour <@${targetId}> !`, ephemeral: true });
}

// ── Cérémonie (élection du nouveau singe) ──────────────────────────────────
  ...
  if (config?.announceChannelId) {
    ...
    if (channel) {
      const embed = new EmbedBuilder()
        ...
        .setTimestamp();
      const pingContent = config?.announceRoleId ? `<@&${config.announceRoleId}> ${EMOJIS.SINGE} Le singe est élu !` : `${EMOJIS.SINGE} Le singe est élu !`;
...
// ── Vérification du mot "singe" sur chaque message ─────────────────────────
...
async function handleMessage(message, client) {
  ...
  if (message.content.toLowerCase().includes('singe')) return; // OK, mot présent
  ...
  try {
    if (faults >= SINGE.FAULT_TIMEOUT_THRESHOLD) {
      ...
      await target.send(`🚨 **Le singe <@${uid}> a oublié de dire singe pour la ${faults}ème fois !** 🐒`);
    } else if (faults >= SINGE.FAULT_SHAME_THRESHOLD) {
      await message.reply(SHAME[Math.floor(Math.random() * SHAME.length)]);
    }
  } catch (err) {
    logger.error('Singe', 'handleMessage error', err);
  }
}
...
module.exports = { nominate, startVote, handleVote, runSingeCeremony, handleMessage };
