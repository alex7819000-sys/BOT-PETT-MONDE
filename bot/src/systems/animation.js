...
async function runRoiDuJour(client, guildId) {
  ...
  const channel = client.guilds.cache.get(guildId)?.channels.cache.get(config?.roiDuJourChannelId || config?.trophyChannelId);
  if (!channel) return;
  ...
  const topUser = await User.findOne({ guildId, dailyXp: { $gt: 0 } })
    .sort({ dailyXp: -1 });
  ...
  // Stats du jour
  const msgToday  = topUser.messagesDay || 0;
  ...
  await User.updateMany({ guildId }, { dailyXp: 0, messagesDay: 0 });
...
module.exports = { runRoiDuJour, postAnnonce, checkPinMessage };
