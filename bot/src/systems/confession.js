...
// ── Bouton/commande "Faire une confession" — ouvre le modal ────────────────
...
async function handleConfessionModal(interaction) {
  ...
  const customTitle = interaction.fields.getTextInputValue('confession_title')?.trim();
  ...
  // Numéro global de la confession (compteur atomique sur la config du serveur)
  const updatedCfg = await Config.findOneAndUpdate(
    { guildId: gid },
    { $inc: { confessionCount: 1 } },
    { upsert: true, new: true }
  );
  const number = updatedCfg.confessionCount;
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle(customTitle ? customTitle : `Confession #${number}`)
    .setDescription(quoted)
    .addFields({ name: '\u200b', value: 'Identité protégée  •  Anonymat garanti' })
    .setFooter({ text: `Confessions Anonymes — Personne ne sait qui tu es | ${formattedDate}` });

  if (cfg?.confessionThumbnailUrl) embed.setThumbnail(cfg.confessionThumbnailUrl);
  ...
  const thread = await msg.startThread({
    name: customTitle ? `🤫 ${customTitle}` : `🤫 Confession #${number} — Réagis ici`,
    ...
    try {
      ...
      if (channel) {
        const embed = new EmbedBuilder()
          ...
          .setTitle(`🕵️ Révélation — Confession #${conf.number ?? ''}`.trim())
