...
// ── Détail jour par jour (pour le graphique + les stats détaillées) ───────
async function getDailyBreakdown(guildId, days = 14) {
  const now = new Date();
  const result = [];
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const [joins, leaves] = await Promise.all([
      MemberLog.countDocuments({ guildId, joinedAt: { $gte: dayStart, $lt: dayEnd } }),
      MemberLog.countDocuments({ guildId, leftAt: { $gte: dayStart, $lt: dayEnd } }),
    ]);

    result.push({ date: dayStart, joins, leaves, net: joins - leaves });
  }
...
module.exports = {
  ...
  getGrowthStats, getInviteLeaderboard, getMemberInviteStats, getDailyBreakdown,
};
