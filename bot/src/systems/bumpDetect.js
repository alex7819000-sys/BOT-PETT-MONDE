...
async function handleMessage(message, client) {
  ...
  // Marque le moment exact de ce bump — permet au rappel (bumpReminder.js) de
  // ping pile 2h après, au lieu d'un horaire fixe qui peut tomber en avance ou
  // en retard selon quand le bump a réellement eu lieu.
  await Config.updateOne({ guildId: gid }, { lastBumpAt: new Date(), bumpReminderSent: false });
  ...
  try {
    await bumper.send(TIPS.bumpRewarded(xpReward, updated.bumpCount));
  } catch { /* DMs fermés, on skip */ }

  // Progression quêtes bump
  require('./quetes').trackBumpProgress(bumper.id, gid, message.guild).catch(() => {});
