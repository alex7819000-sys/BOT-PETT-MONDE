  ...
  try {
    const response = await axios.post(
      `${API_URL}?key=${process.env.GEMINI_API_KEY}`,
      {
        ...
        systemInstruction: { parts: [{ text: buildSystemInstruction(message.guild) }] },
    ...
    logger.error('AIChat', 'Erreur appel Gemini', err?.response?.data || err.message);
    await message.reply('😵 J\'ai eu un souci pour répondre, réessaie dans un instant.').catch(() => {});
  }
