...
// ── Petit message spontané envoyé à un nouveau membre pour engager la conversation ──
function buildWelcomeSystemInstruction(guild, memberName) {
  return (
    `Tu es le bot officiel du serveur Discord "${guild.name}", mais tu t'exprimes avec un ton naturel ` +
    `et familier, comme le ferait n'importe quel membre actif du serveur. ${memberName} vient tout juste ` +
    `de rejoindre le serveur. Envoie-lui un message très court et spontané pour engager la conversation ` +
    `et le mettre à l'aise (par exemple lui demander comment il va, ce qui l'amène, un truc simple et ` +
    `chaleureux) — pas un message d'accueil formel de bot, plutôt le genre de truc que dirait un membre ` +
    `sympa en voyant quelqu'un débarquer. 1 à 2 phrases maximum, un emoji si ça vient naturellement. ` +
    `Ne prétends jamais être humain si on te le demande directement — assume que t'es une IA si la ` +
    `question est posée franchement, mais garde ta personnalité de membre du serveur le reste du temps.`
  );
}
...
// ── Si la réponse est coupée en plein milieu d'une phrase (limite de tokens
// atteinte), on retrouve la fin de la dernière phrase complète plutôt que de
// laisser un mot à moitié écrit — beaucoup moins visible qu'une vraie coupure.
function cleanTruncatedReply(text) {
  const trimmed = text.trim();
  // Se termine déjà proprement (ponctuation de fin de phrase ou emoji) → rien à faire.
  // L'apostrophe est volontairement exclue : "aujourd'" coupé en plein milieu d'un mot
  // ("aujourd'hui") se terminerait aussi par une apostrophe, donc ce n'est pas un
  // indicateur fiable de phrase complète.
  if (/[.!?…»😀-🿿]$/u.test(trimmed)) return trimmed;

  const lastEnd = Math.max(trimmed.lastIndexOf('.'), trimmed.lastIndexOf('!'), trimmed.lastIndexOf('?'));
  // On ne coupe que si ça laisse une réponse substantielle (pas juste 2 mots) —
  // sinon autant garder la phrase incomplète plutôt que quasi rien.
  if (lastEnd > trimmed.length * 0.3) return trimmed.slice(0, lastEnd + 1);
  // Aucun point de coupure propre trouvé (réponse coupée avant la 1ère phrase complète) —
  // au moins signaler visuellement que c'est incomplet plutôt que de laisser un mot tronqué.
  return trimmed + '…';
}
...
async function handleAIChat(message, client) {
  ...
  try {
    ...
    reply = cleanTruncatedReply(reply);
...
// ── Envoie un message spontané à un nouveau membre pour engager la conversation ──
// Appelé depuis index.js (guildMemberAdd) en plus du ping Chat Revive. Silencieux
// en cas d'échec (pas de clé API, quota épuisé, erreur réseau) — ce n'est qu'un
// bonus, pas une fonctionnalité critique, donc ça ne doit jamais faire échouer
// le reste du flux d'arrivée d'un membre.
async function greetNewMember(member, channel) {
  if (!process.env.GEMINI_API_KEY) return false;
  const guild = member.guild;

  try {
    const cfg = await Config.findOne({ guildId: guild.id }).lean().catch(() => null);
    if (!canCallNow()) return false;
    const allowed = await checkAndConsumeQuota(guild.id, cfg || {});
    if (!allowed) return false;

    const memberName = member.displayName || member.user.username;
    const channelId = channel.id;
    const prompt = `(${memberName} vient tout juste de rejoindre le serveur)`;

    const response = await axios.post(
      `${API_URL}?key=${process.env.GEMINI_API_KEY}`,
      {
        contents: [{ role: 'user', parts: [{ text: prompt }] }],
        systemInstruction: { parts: [{ text: buildWelcomeSystemInstruction(guild, memberName) }] },
    ...
    // Mémorise l'échange pour que la conversation se poursuive naturellement dans
    // ce salon si le nouveau membre répond au message (même mémoire que handleAIChat).
    pushToHistory(channelId, 'user', prompt);
    pushToHistory(channelId, 'model', reply);

    await channel.send({ content: `<@${member.id}> ${reply}` }).catch(() => {});
    return true;
  } catch (err) {
    logger.error('AIChat', 'Erreur greet nouveau membre (après retries éventuels)', err?.response?.data || err.message);
    return false;
  }
}

module.exports = { handleAIChat, greetNewMember };
