...
function formatBoard(entries, guild) {
  ...
  return entries
    .map((e, i) => {
      ...
      return `${tag} <@${e.userId}> — **${e.score} pts** (✅ ${e.good} · ❌ ${e.faults} · 🔍 ${e.caught} · 🎭 ${e.won})`;
    })
    .join('\n');
}
...
async function postIntermediateLeaderboard(client) {
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      if (!top.length) continue; // personne n'a participé, on n'envoie rien

      const embed = new EmbedBuilder()
        .setColor(0x5865F2)
        .setTitle('📊 Classement Counting — En direct')
        .setDescription(formatBoard(top, guild))
        .setFooter({ text: 'Score = bons chiffres − fautes + bluffs démasqués + bluffs réussis ×2 · Classement final à minuit' })
        .setTimestamp();
...
async function postFinalLeaderboardAndCrown(client) {
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      // Si personne n'a participé aujourd'hui, on n'envoie aucun message,
      // on se contente de reset les stats en silence.
      if (!top.length) {
        await resetDailyCountingStats(guild.id);
        await Config.updateOne({ guildId: guild.id }, { countingCurrent: 0, countingLastUserId: null });
        continue;
      }
      ...
      // Multiplicateur du jour — tiré au hasard parmi les paliers configurés (x6/x12/x24 par défaut).
      // Renouvelé chaque jour, donc le bonus du Champion n'est jamais le même 2 jours de suite.
      const tiers = cfg?.countingXpBonusMultipliers?.length ? cfg.countingXpBonusMultipliers : [6, 12, 24];
      const dailyMultiplier = tiers[Math.floor(Math.random() * tiers.length)];
      await Config.updateOne({ guildId: guild.id }, { countingDailyMultiplier: dailyMultiplier });
      ...
      if (channel) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(formatBoard(top, guild))
          .setFooter({ text: 'Score = bons chiffres − fautes · Classement remis à zéro chaque jour' })
          .setTimestamp();
          ...
          embed.addFields({
            ...
            value: `<@${winner.userId}> avec **${winner.score} points** !\n` +
              (cfg.countingChampionRoleId
                ? `Reçoit le rôle <@&${cfg.countingChampionRoleId}> → **×${dailyMultiplier} XP pendant 24h** ! 🎲`
      ...
      if (winner && winner.score > 0 && cfg.countingChampionRoleId) {
        ...
        if (member && role) {
          ...
          logger.info('CountingLeaderboard', `${winner.userId} devient Champion du Counting (×${dailyMultiplier} XP, ${winner.score} pts)`);
        }
      }
      ...
      // Le compteur lui-même ne reset QUE ici, jamais sur une simple faute en journée —
      // c'est le seul moment où la partie redémarre vraiment à 0.
      await Config.updateOne({ guildId: guild.id }, { countingCurrent: 0, countingLastUserId: null });
    } catch (err) {
...
async function getCountingXpMultiplier(userId, guildId) {
  ...
  // Multiplicateur tiré au hasard à chaque couronnement (×6 / ×12 / ×24 par défaut) —
  // pas un pourcentage fixe, pour que le bonus du jour soit toujours une surprise.
  return cfg?.countingDailyMultiplier ?? 6;
}
