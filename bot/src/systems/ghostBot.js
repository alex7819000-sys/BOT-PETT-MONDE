...
// guildId → true pendant qu'un join est en cours — évite qu'un 2e appel concurrent
// (ex: reconnexion auto au démarrage + /vocal join lancé en même temps) ne détruise
// la connexion en cours de négociation et ne fausse le diagnostic.
const joining = new Set();

async function joinGhost(client, guildId, channelId) {
  if (joining.has(guildId)) {
    return { ok: false, reason: 'Une connexion est déjà en cours pour ce serveur, réessaie dans quelques secondes.' };
  }
  joining.add(guildId);
  try {
    return await doJoinGhost(client, guildId, channelId);
  } finally {
    joining.delete(guildId);
  }
}

async function doJoinGhost(client, guildId, channelId) {
  ...
  // Logs détaillés — pour voir précisément où ça bloque en cas de souci
  // (Render → Logs, au moment du /vocal join, on doit voir cette séquence d'états)
  connection.on('stateChange', (oldS, newS) => {
    logger.info('GhostBot', `[${guildId}] État connexion vocale : ${oldS.status} → ${newS.status}`);
  });
  connection.on('debug', (msg) => {
    logger.debug('GhostBot', `[${guildId}] ${msg}`);
  });
  connection.on('error', (err) => {
    logger.error('GhostBot', `[${guildId}] Erreur connexion vocale`, err);
  });
  ...
  try {
    ...
    connections.delete(guildId);
    connection.destroy();
    logger.error('GhostBot', `Timeout connexion vocale (guild ${guildId}, salon ${channelId}) — état bloqué à: ${connection.state?.status}`, err);
    return {
      ok: false,
      reason: 'Connexion au vocal impossible (timeout). Si ça persiste à chaque essai, c\'est probablement l\'hébergeur (Render) qui bloque/limite le trafic UDP nécessaire à la voix Discord — pas un bug du bot.',
    };
  }
