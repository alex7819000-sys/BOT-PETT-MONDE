...
async function handleMessage(message) {
  ...
  // Poster l'image
  await postFactionImage(message, triggered);
  return true;
}

async function postFactionImage(message, faction) {
  try {
    ...
    const embed = new EmbedBuilder()
      .setColor(faction.keyword === 'chien' ? 0x8B4513 : faction.keyword === 'chat' ? COLORS.PINK : COLORS.GOLD)
      .setTitle(`${faction.emoji} ${faction.name.toUpperCase()} !`)
      .setFooter({ text: `Faction ${faction.name} • écris "${faction.keyword}" pour rejoindre !` });

    if (imageUrl) embed.setImage(imageUrl);

    await message.channel.send({ embeds: [embed] });
  } catch {
    await message.react(faction.emoji).catch(() => {});
  }
}
