...
const TIPS = {
  ...
  podiumResult: (rank, xpToday, msgsToday) =>
    `🌙 **Podium du jour — tu étais #${rank} !**\n` +
    `> Aujourd'hui tu as envoyé **${msgsToday} messages** et gagné **${xpToday.toLocaleString('fr-FR')} XP**.\n` +
    `> Le classement se remet à zéro chaque nuit — reviens demain pour viser le top !\n` +
    `> 👉 Tape \`/top\` pour voir le classement général.`,
