...
async function handleMessage(message) {
  ...
  if (bluffIntents.has(userId)) {
    ...
    // Boutons sous le message (le 🎭 est en rouge pour indiquer visuellement un bluff potentiel — mais les joueurs ne savent pas si c'est vraiment un bluff ou juste la couleur)
    await message.edit({ components: [buildButtons(message.id, false)] }).catch(async () => {
      // Si edit échoue (permissions), on envoie un follow-up avec les boutons
      await message.channel.send({ content: `⬆️ Message de ${message.author}`, components: [buildButtons(message.id)] }).catch(() => {});
    });
  ...
  // Boutons sous le message
  try {
    await message.edit({ components: [buildButtons(message.id)] });
  } catch {
    // message.edit peut échouer si le message n'est pas éditable (webhooks, etc.)
    // Dans ce cas on réagit comme fallback
