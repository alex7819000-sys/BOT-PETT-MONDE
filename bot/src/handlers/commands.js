  ...
  return [
    ...
    new SlashCommandBuilder().setName('anime')
      ...
      .addSubcommand(s => s.setName('now').setDescription('Poster un perso maintenant')
        .addStringOption(o => o.setName('image').setDescription('URL de l\'image du personnage').setRequired(true))
        .addStringOption(o => o.setName('nom').setDescription('Nom du personnage').setRequired(false)))
    ...
    // /niveau — profil rapide avec missions du jour + défis
    new SlashCommandBuilder().setName('niveau').setDescription('Voir ton niveau, missions du jour et défis actifs')
      .addUserOption(o => o.setName('membre').setDescription('Voir le niveau d\'un autre membre').setRequired(false)),

    // /top — top 10 classement XP rapide
    new SlashCommandBuilder().setName('top').setDescription('🏆 Top 10 du classement XP du serveur')
    ...
    new SlashCommandBuilder().setName('pub')
      ...
      .addSubcommand(s => s.setName('toggle').setDescription('Activer/désactiver une pub')
        .addStringOption(o => o.setName('id').setDescription('ID de la pub').setRequired(true)))
      .addSubcommand(s => s.setName('panel').setDescription('Poster le panel "Demander une publication"')
        .addChannelOption(o => o.setName('salon').setDescription('Salon où poster le panel').setRequired(false))),

    // /7777 — Table aléatoire avec collection, combos, et classement
    new SlashCommandBuilder().setName('7777')
      .setDescription('🎰 Roulette aléatoire — Collecte des chiffres, débloque des combos')
      .addSubcommand(s => s.setName('roll').setDescription('🎲 Faire un tirage'))
      .addSubcommand(s => s.setName('collection').setDescription('📚 Voir ta collection personnelle'))
      .addSubcommand(s => s.setName('leaderboard').setDescription('🏆 Classement global des jetons'))
      .addSubcommand(s => s.setName('semaine').setDescription('📅 Classement de la semaine')),
    ...
    // /notif
    new SlashCommandBuilder().setName('partager')
    ...
    // /setup
    // /faction — système de factions custom + chien/chat
    new SlashCommandBuilder().setName("faction").setDescription("Système de factions du salon bataille")
      .addSubcommand(s => s.setName("liste").setDescription("Voir toutes les factions actives"))
      .addSubcommand(s => s.setName("créer").setDescription("Créer une nouvelle faction (coûte 500 XP)")
        .addStringOption(o => o.setName("nom").setDescription("Nom de la faction").setRequired(true))
        .addStringOption(o => o.setName("motcle").setDescription("Mot-clé déclencheur (min 3 lettres, unique)").setRequired(true))
        .addStringOption(o => o.setName("image").setDescription("URL de l'image postée à chaque trigger").setRequired(false))
        .addStringOption(o => o.setName("emoji").setDescription("Emoji représentant la faction").setRequired(false)))
      .addSubcommand(s => s.setName("supprimer").setDescription("Supprimer une faction (créateur ou admin)")
        .addStringOption(o => o.setName("motcle").setDescription("Mot-clé de la faction à supprimer").setRequired(true))),

    // /quete — quêtes à XP (quotidiennes auto + créées par les modos/admins)
    // Note : la vérification "modo/admin requis" pour creer/terminer/recompenser se fait dans le handler
    // (Discord ne permet pas de restreindre une sous-commande individuellement par permission).
    new SlashCommandBuilder().setName('quete')
      .setDescription('Quêtes à XP du serveur')
      .addSubcommand(s => s.setName('creer')
        .setDescription('Créer une quête (modo/admin)')
        .addStringOption(o => o.setName('titre').setDescription('Titre de la quête').setRequired(true))
        .addIntegerOption(o => o.setName('xp').setDescription('XP à gagner').setRequired(true).setMinValue(10).setMaxValue(100000))
        .addStringOption(o => o.setName('type').setDescription('Type de quête').setRequired(true).addChoices(
          { name: '💬 Messages dans un salon précis', value: 'messages_channel' },
          { name: '📊 Messages au total (tous salons)', value: 'messages_total' },
          { name: '🚨 Urgente — premier à atteindre la cible', value: 'first_to_messages' },
          { name: '🏆 Concours (vidéo/image la plus réagie)', value: 'contest' },
          { name: '🚀 Bump le serveur', value: 'bump' },
          { name: '🎙️ Minutes en vocal', value: 'vocal_minutes' },
        ))
        .addIntegerOption(o => o.setName('cible').setDescription('Nombre cible à atteindre (messages, bump, minutes...)').setRequired(false).setMinValue(1))
        .addChannelOption(o => o.setName('salon').setDescription('Salon concerné (pour "messages dans un salon" ou "urgente")').setRequired(false))
        .addIntegerOption(o => o.setName('duree_jours').setDescription('Durée en jours avant la fin (défaut: 7)').setRequired(false).setMinValue(1).setMaxValue(60))
        .addIntegerOption(o => o.setName('bonus').setDescription('Bonus XP temporaire en récompense (+25%, +50% ou +100%)').setRequired(false).addChoices(
          { name: '⭐ +25% XP', value: 25 },
          { name: '🌟 +50% XP', value: 50 },
          { name: '💫 +100% XP', value: 100 },
        ))
        .addIntegerOption(o => o.setName('kakera').setDescription('Kakera Mudae en récompense (optionnel)').setRequired(false).setMinValue(1).setMaxValue(10000)))
      .addSubcommand(s => s.setName('liste').setDescription('Voir les quêtes actives'))
      .addSubcommand(s => s.setName('terminer')
        .setDescription('Clôturer une quête manuellement (admin)')
        .addStringOption(o => o.setName('id').setDescription('ID de la quête (voir /quete liste)').setRequired(true)))
      .addSubcommand(s => s.setName('recompenser')
        .setDescription('Donner manuellement la récompense d\'une quête à quelqu\'un (admin)')
        .addStringOption(o => o.setName('id').setDescription('ID de la quête').setRequired(true))
        .addUserOption(o => o.setName('membre').setDescription('Membre à récompenser').setRequired(true)))
      .addSubcommand(s => s.setName('bonus')
        .setDescription('Voir les bonus XP actifs d\'un membre')
        .addUserOption(o => o.setName('membre').setDescription('Membre (toi par défaut)').setRequired(false))),

    // ── /relance ──────────────────────────────────────────────────────────────
    new SlashCommandBuilder().setName('relance')
      .setDescription('📬 Renvoyer le DM de bienvenue aux membres inactifs')
      .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
      .addSubcommand(s => s.setName('tous')
        .setDescription('Envoyer le DM à TOUS les membres sans exception')
        .addStringOption(o => o.setName('couleur').setDescription('Couleur de l\'embed').setRequired(false)
          .addChoices(
      ...
      .addSubcommand(s => s.setName('membre')
        .setDescription('Renvoyer le DM à un membre précis')
        .addUserOption(o => o.setName('cible').setDescription('Membre à relancer').setRequired(true))
        .addStringOption(o => o.setName('couleur').setDescription('Couleur de l\'embed (ex : Rose, Bleu, Vert...)').setRequired(false)
          .addChoices(
            { name: '🔴 Rouge',  value: 'Rouge'  },
            { name: '🟠 Orange', value: 'Orange' },
            { name: '🟡 Jaune',  value: 'Jaune'  },
      ...
      .addSubcommand(s => s.setName('inactifs')
        .setDescription('Envoyer à tous les membres inactifs depuis X jours')
        .addIntegerOption(o => o.setName('jours').setDescription('Inactifs depuis combien de jours (défaut : 7)').setRequired(false).setMinValue(1).setMaxValue(90))
        .addStringOption(o => o.setName('couleur').setDescription('Couleur de l\'embed').setRequired(false)
          .addChoices(
            { name: '🔴 Rouge',  value: 'Rouge'  },
            { name: '🟠 Orange', value: 'Orange' },
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand(s => s.setName('smash')
        .setDescription('Choisir les 2 emoji du Smash or Pass')
        .addStringOption(o => o.setName('smash').setDescription('Emoji pour "Smash" (ex: 🔥)').setRequired(false).setMaxLength(10))
        .addStringOption(o => o.setName('pass').setDescription('Emoji pour "Pass" (ex: 💀)').setRequired(false).setMaxLength(10)))
      .addSubcommand(s => s.setName('confession')
        .setDescription('Configurer le délai avant révélation de l\'auteur d\'une confession')
        .addIntegerOption(o => o.setName('heures').setDescription('Délai en heures avant révélation (défaut: 48)').setRequired(true).setMinValue(1).setMaxValue(720)))
    ...
    // /ticket
    new SlashCommandBuilder().setName('ticket').setDescription('Système de tickets')
      .addSubcommand(s => s.setName('ouvrir').setDescription('Ouvrir un ticket')
        .addStringOption(o => o.setName('type').setDescription('Type de ticket').setRequired(false)
          .addChoices(
      ...
      .addSubcommand(s => s.setName('liste').setDescription('Voir les tickets ouverts'))
      .addSubcommand(s => s.setName('panel').setDescription('(Admin) Envoyer le panel ticket dans un salon')
        .addChannelOption(o => o.setName('salon').setDescription('Salon où poster le panel').setRequired(false))),
...
async function handleCommand(interaction, client) {
  ...
  try {
    switch (cmd) {
      ...
      case 'waifu':
      case 'animaux': {
        return interaction.reply({ content: '❌ Ce système a été remplacé par les forums **#anime**, **#gaming** et **#musique**.', ephemeral: true });
      }
      ...
      case 'ticket':      return require('../systems/ticket').handleTicketCommand(interaction, client);
      case 'embed':       return require('./commandHandlers/embed').handle(interaction);
      case 'missions':    return require('./commandHandlers/missions')(interaction);
      case 'niveau':      return require('./commandHandlers/niveau')(interaction);
      case 'top':         return require('./commandHandlers/top')(interaction);
      ...
      // ── Quiz ────────────────────────────────────────────────────────
      case 'quiz':        return interaction.reply({ content: '❌ Le quiz anime a été remplacé par le forum **#anime**.', ephemeral: true });
      ...
      case 'partager':    return require('./commandHandlers/notif').handle(interaction);
      ...
      case 'relance':     return require('./commandHandlers/relance').handle(interaction);

      // ── Setup ───────────────────────────────────────────────────────
      case 'setup':       return require('./commandHandlers/setup').handle(interaction, client);
      case 'quete':       return require('./commandHandlers/quetes').handle(interaction);
      case 'faction':     return require('./commandHandlers/faction')(interaction);
      ...
      // ── Table 7777 ──────────────────────────────────────────────────
      case '7777': {
        const table7777 = require('../systems/table7777');
        const sub = interaction.options.getSubcommand();
        if (sub === 'roll') {
          return require('./commandHandlers/table7777').handleRoll(interaction);
        }
        if (sub === 'collection') {
          return require('./commandHandlers/table7777').handleCollection(interaction);
        }
        if (sub === 'leaderboard') {
          return require('./commandHandlers/table7777').handleLeaderboard(interaction);
        }
        if (sub === 'semaine') {
          return require('./commandHandlers/table7777').handleWeekly(interaction);
        }
        break;
      }
