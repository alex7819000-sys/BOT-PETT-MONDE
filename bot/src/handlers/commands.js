...
function buildCommands() {
  return [
    ...
    new SlashCommandBuilder().setName('guerre')
      ...
      .addSubcommand(s => s.setName('membres').setDescription('Top des membres les plus actifs')),

    // /signaler — Signaler un membre avec preuve (part en validation staff)
    new SlashCommandBuilder().setName('signaler')
      .setDescription('Signaler un membre au staff avec une preuve (avertissement/mute/kick/ban)')
      .addUserOption(o => o.setName('membre').setDescription('Membre à signaler').setRequired(true))
      .addStringOption(o => o.setName('raison').setDescription('Raison du signalement').setRequired(true).setMaxLength(500))
      .addAttachmentOption(o => o.setName('preuve_image').setDescription('Capture d\'écran ou image de preuve').setRequired(false))
      .addStringOption(o => o.setName('preuve_texte').setDescription('Preuve en texte ou lien (si pas d\'image)').setRequired(false).setMaxLength(500)),
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand(s => s.setName('sanction')
        ...
        .addChannelOption(o => o.setName('salon').setDescription('Salon où postent les demandes en attente de validation').setRequired(false))
...
async function handleCommand(interaction, client) {
  ...
  try {
    switch (cmd) {
      ...
      // ── Signalement / Sanctions ────────────────────────────────────
      case 'signaler':    return require('./commandHandlers/signaler').handle(interaction);
