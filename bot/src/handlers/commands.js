...
function buildCommands() {
  return [
    ...
    new SlashCommandBuilder().setName('stats')
      ...
      .addSubcommand(s => s.setName('croissance')
        .setDescription('Arrivées/départs détaillés, avec graphique')
        .addStringOption(o => o.setName('periode').setDescription('Nombre de jours affichés sur le graphique (défaut: 14)').setRequired(false)
          .addChoices(
            { name: '7 jours', value: '7' },
            { name: '14 jours', value: '14' },
            { name: '30 jours', value: '30' },
          )))
      .addSubcommand(s => s.setName('actifs').setDescription('Top des membres les plus actifs (XP total, tout confondu)'))
      .addSubcommand(s => s.setName('vocal').setDescription('Top des membres qui passent le plus de temps en vocal'))
      .addSubcommand(s => s.setName('salons').setDescription('Quels salons sont les plus utilisés')),
    ...
    new SlashCommandBuilder().setName('setup')
      ...
      .addSubcommand(s => s.setName('salon')
        ...
        .addStringOption(o => o.setName('type').setDescription('Type de salon').setRequired(true).addChoices(
          ...
          { name: '💬 Chat Revive (ping à l\'arrivée)',  value: 'chatReviveChannelId' },
        ))
      ...
      .addSubcommand(s => s.setName('role')
        ...
        .addStringOption(o => o.setName('type').setDescription('Type de rôle').setRequired(true).addChoices(
          ...
          { name: 'Ping Bump 🚀',      value: 'bumperRoleId' },
          ...
          { name: '💬 Ping Chat Revive', value: 'chatReviveRoleId' },
          { name: '👑 King of Hera (trophée hebdo, top XP semaine)', value: 'kingOfHeraRoleId' },
        ))
      ...
      .addSubcommand(s => s.setName('chatrevive')
        .setDescription('Configurer le Chat Revive (salon, rôle et délai en un seul coup)')
        .addChannelOption(o => o.setName('salon').setDescription('Salon où poster le ping').setRequired(false))
        .addRoleOption(o => o.setName('role').setDescription('Rôle à ping à chaque nouvelle arrivée').setRequired(false))
      ...
      .addSubcommand(s => s.setName('syncroles')
        .setDescription('(Admin) Réattribue les rôles de niveau à TOUS les membres selon leur niveau actuel (rattrapage)'))
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand(s => s.setName('maj')
        .setDescription('Salon où le bot annonce ses mises à jour')
        .addChannelOption(o => o.setName('salon').setDescription('Salon des annonces de mise à jour').setRequired(true)))
      .addSubcommand(s => s.setName('ia')
        .setDescription('Configurer le chat IA (répond quand on le mentionne ou dit son nom)')
        .addStringOption(o => o.setName('nom').setDescription('Nom que les membres disent pour lui parler (ex: sayen)').setRequired(true))
        .addChannelOption(o => o.setName('salon').setDescription('Salon où le bot peut discuter (vide = partout sur le serveur)').setRequired(false))
        .addBooleanOption(o => o.setName('partout').setDescription('Mettre à true pour repasser à "répond partout" (annule un salon déjà configuré)').setRequired(false))
        .addIntegerOption(o => o.setName('limite').setDescription('Nb max de réponses/jour (défaut: 200, marge de sécurité sous le quota gratuit Gemini)').setRequired(false).setMinValue(10).setMaxValue(1400)))
