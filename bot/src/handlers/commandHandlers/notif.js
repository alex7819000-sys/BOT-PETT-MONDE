// src/handlers/commandHandlers/notif.js — /notif (config rapide : maj, counting, confession, sanction)
'use strict';
...
async function handle(interaction) {
  ...
  if (sub === 'maj') {
    ...
    await Config.updateOne({ guildId: gid }, { botUpdatesChannelId: salon.id });
    return interaction.editReply({ content: `🤖 Réglé — les mises à jour du bot seront annoncées dans ${salon}.` });
  }

  if (sub === 'ia') {
    const nom = interaction.options.getString('nom');
    const salon = interaction.options.getChannel('salon');
    const partout = interaction.options.getBoolean('partout');
    const limite = interaction.options.getInteger('limite');
    const update = { aiChatTriggerName: nom };
    if (salon) update.aiChatChannelId = salon.id;
    else if (partout) update.aiChatChannelId = null;
    if (limite) update.aiChatDailyLimit = limite;
    await Config.updateOne({ guildId: gid }, update);
    return interaction.editReply({
      content: `🤖 Chat IA activé — les membres peuvent me parler en me mentionnant ou en disant **"${nom}"**` +
        (salon ? ` dans ${salon} uniquement.` : partout ? ` dans n'importe quel salon.` : ` (salon : celui déjà configuré, ou partout si aucun réglé).`) +
        (limite ? ` Limite : **${limite}** réponses/jour.` : ''),
    });
  }
