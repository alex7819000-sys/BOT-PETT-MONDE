...
async function handle(interaction, client) {
  ...
  // ── /setup chatrevive — salon + rôle + délai, en une seule commande ─────
  if (sub === 'chatrevive') {
    const salon   = interaction.options.getChannel('salon');
    const role    = interaction.options.getRole('role');
    const minutes = interaction.options.getInteger('minutes');
    const update = {};
    if (salon)   update.chatReviveChannelId = salon.id;
    if (role)    update.chatReviveRoleId = role.id;
    if (minutes) update.chatReviveMinutes = minutes;
    if (!Object.keys(update).length) {
      return interaction.editReply({ content: `ℹ️ Configuration actuelle — Salon : ${config.chatReviveChannelId ? `<#${config.chatReviveChannelId}>` : 'non réglé'} · Rôle : ${config.chatReviveRoleId ? `<@&${config.chatReviveRoleId}>` : 'non réglé'} · Minutes : ${config.chatReviveMinutes || 90}` });
    }
    await Config.updateOne({ guildId: gid }, update);
    const lignes = [];
    if (salon) lignes.push(`Salon : <#${salon.id}>`);
    if (role) lignes.push(`Rôle : <@&${role.id}>`);
    if (minutes) lignes.push(`Minutes : ${minutes}`);
    return interaction.editReply({ content: `✅ Chat Revive mis à jour.\n${lignes.join('\n')}` });
  }
  ...
  // ── /setup syncroles — rattrapage : réapplique les rôles de niveau à tous les membres ──
  // Utile si des membres ont déjà leur niveau en base (XP gagné avant qu'un palier
  // existe, rôle retiré par erreur, migration...) sans jamais retrigger un vrai
  // level up ensuite — applyLevelRoles ne tourne normalement qu'au moment où le
  // niveau change, donc ces membres restaient bloqués sans leur rôle jusqu'ici.
  if (sub === 'syncroles') {
    const cfgFresh = await Config.findOne({ guildId: gid }).lean();
    if (!cfgFresh?.levelRoles?.length) {
      return interaction.editReply({ content: '⚠️ Aucun palier de niveau configuré (`/setup levelrole`) — rien à synchroniser.' });
    }

    const { applyLevelRoles } = require('../../systems/roleRewards');
    const User = require('../../db/models/User');

    const users = await User.find({ guildId: gid }).lean();

    // guild.members.fetch() peut rester bloqué indéfiniment sur un souci réseau
    // ponctuel (Render, gateway Discord) au lieu de rejeter — un simple .catch()
    // ne suffit pas dans ce cas : la promesse ne se résout jamais, donc
    // editReply() n'est jamais appelé et la commande tourne en chargement infini
    // côté Discord jusqu'à expiration du token (15 min). On force donc un
    // timeout et on se rabat sur le cache existant (déjà à jour sur un petit
    // serveur avec l'intent GuildMembers actif) si le fetch ne répond pas.
    let members;
    try {
      ...
      logger.warn('Setup', `syncroles : fetch() a échoué/timeout (${err.message}), repli sur le cache existant`);
      members = interaction.guild.members.cache;
    }
    if (!members || members.size === 0) {
      return interaction.editReply({ content: '❌ Impossible de récupérer la liste des membres du serveur (cache vide également).' });
    }

    let processed = 0;
    for (const u of users) {
      const member = members.get(u.userId);
      if (!member) continue; // membre parti du serveur entre-temps
      const totalXp = u.totalXp || 0;
      const level = Math.floor(0.1 * Math.sqrt(totalXp));
      if (level !== u.level) await User.updateOne({ userId: u.userId, guildId: gid }, { level });
      await applyLevelRoles(member, cfgFresh, level);
      processed++;
    }

    return interaction.editReply({ content: `✅ Synchronisation terminée — **${processed}** membre(s) vérifié(s), rôles de niveau réattribués selon le niveau actuel de chacun.` });
  }
