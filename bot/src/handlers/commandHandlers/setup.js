...
async function handle(interaction, client) {
  ...
  // ── /setup chatrevive — réglage du délai d'inactivité (salon/rôle : /setup salon et /setup role) ──
  if (sub === 'chatrevive') {
    const minutes = interaction.options.getInteger('minutes');
    if (!minutes) {
      return interaction.editReply({ content: `ℹ️ Délai actuel : **${config.chatReviveMinutes || 90} minutes**. Pour changer le salon ou le rôle, utilise \`/setup salon\` (type 💬 Chat Revive) et \`/setup role\` (type 💬 Ping Chat Revive).` });
    }
    await Config.updateOne({ guildId: gid }, { chatReviveMinutes: minutes });
    return interaction.editReply({ content: `✅ Chat Revive : délai réglé sur **${minutes} minutes**.` });
  }
