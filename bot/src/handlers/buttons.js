...
async function handleButton(interaction, client) {
  ...
  if (ns === 'levelup' && action === 'info') {
    try {
      ...
      logger.error('Buttons', `[${id}] Error:`, err?.message || err);
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({ content: '❌ Erreur levelup. ' + (err?.message || ''), ephemeral: true }).catch(() => {});
      }
    }
    return;
  }
  ...
  try {
    switch (ns) {
      ...
      case 'welcome': {
        if (action === 'section') {
          const { handleWelcomeSection } = require('../systems/welcomeInteractive');
          return handleWelcomeSection(interaction);
        }
        if (action === 'action') {
          const { handleWelcomeAction } = require('../systems/welcomeInteractive');
          return handleWelcomeAction(interaction);
        }
        break;
      }
      case 'reglement': {
        ...
        if (action === 'section') {
          const { handleReglementSection } = require('../systems/reglement');
          return handleReglementSection(interaction);
        }
        break;
      }
      case 'confession': {
        ...
        const infos = {
          ...
          xp: {
            ...
            lines: [
              ...
              '🎙️ Temps en vocal → **+XP** (toutes les 5 min, bonus fidélité)',
              '🚀 Bump Disboard → **+500 XP**',
              '📋 Présentation complète → **bonus XP**',
              '🔢 Counting → points + rôle **Champion du Counting** si t\'es le meilleur du jour',
              '📖 Quêtes quotidiennes → **grosses récompenses**',
              '🏆 **Champions du Jour** — chaque nuit, le plus actif à l\'écrit et le plus actif en vocal reçoivent chacun un rôle',
            ],
          },
          regles: {
      ...
      case 'sanction': {
        const sanctions = require('../systems/sanctions');
        const requestId = rest[0];
        if (action === 'validate')   return sanctions.handleValidateButton(interaction, requestId);
        if (action === 'changetier') return sanctions.handleChangeTierButton(interaction, requestId);
        if (action === 'reject')     return sanctions.handleRejectButton(interaction, requestId);
        if (action === 'tier')       return sanctions.handleTierSelect(interaction, requestId);
        if (action === 'duration')   return sanctions.handleDurationSelect(interaction, requestId, rest[1]);
        break;
      }
      case 'sigrep': {
        const reportPanel = require('../systems/reportPanel');
        if (action === 'start')      return reportPanel.handleStartButton(interaction);
        if (action === 'selectuser') return reportPanel.handleUserSelected(interaction);
        if (action === 'tier')       return reportPanel.handleTierSelected(interaction, rest[0]);
        if (action === 'duration')   return reportPanel.handleDurationSelected(interaction, rest[0]);
        break;
      }
      case 'ticket': {
    ...
    logger.error('Buttons', `[${ns}:${action}] Erreur:`, err?.message || err);
    console.error('Full error:', err); // Log complet pour déboguer
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: `❌ Erreur: ${err?.message || 'Unknown error'}`.substring(0, 100), ephemeral: true }).catch(() => {});
    } else if (interaction.deferred && !interaction.replied) {
      await interaction.editReply({ content: `❌ Erreur: ${err?.message || 'Unknown error'}`.substring(0, 100) }).catch(() => {});
    }
  }
}
