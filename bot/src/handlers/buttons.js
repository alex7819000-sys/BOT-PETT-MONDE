...
async function handleButton(interaction, client) {
  ...
  // ── Counting buttons (format: counting_verify_MSGID / counting_bluff_MSGID) ──
  if (id.startsWith('counting_')) {
    try {
      const { handleButton: countingBtn } = require('../systems/counting');
      await countingBtn(interaction);
    } catch (err) {
      logger.error('Buttons', `Error handling counting button ${id}`, err);
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({ content: '❌ Une erreur est survenue.', ephemeral: true }).catch(() => {});
      }
    }
