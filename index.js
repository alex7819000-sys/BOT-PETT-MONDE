...
client.on('messageCreate', async message => {
  ...
  try {
    ...
    if (newLevel > (updated.level || 0)) {
      ...
      // Rôle de niveau (swap : enlève l'ancien, donne le nouveau)
      ...
      if (cfg?.levelRoles?.length) {
        ...
        const match = allSorted.find(lr => lr.level <= newLevel);
        const member = message.member;
        if (match && member) {
          // Retire tous les anciens rôles de niveau
          for (const lr of allSorted) {
            if (lr.roleId !== match.roleId && member.roles.cache.has(lr.roleId)) {
              member.roles.remove(lr.roleId).catch(() => {});
            }
          }
          // Donne le nouveau
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (oldState.channelId && !newState.channelId) {
    ...
    // ── Tip XP vocal en DM ─────────────────────────────────────────────────
    try {
      const vocalXpPerMin = 5; // estimation : ~5 XP/min en vocal
      const xpGained = minutes * vocalXpPerMin;
      const member = newState.guild?.members.cache.get(uid) || oldState.guild?.members.cache.get(uid);
      if (member && minutes >= 2) { // seulement si session significative
        const { sendTip, TIPS } = require('./bot/src/systems/tips');
        await member.user.send(TIPS.vocalXp(minutes, xpGained)).catch(() => {});
      }
    } catch { /* DMs fermés */ }

    // Progression quêtes vocal
    const guild = newState.guild || oldState.guild;
    if (guild) {
      require('./bot/src/systems/quetes').trackVocalProgress(uid, gid, guild, minutes).catch(() => {});
    }
  }
});
