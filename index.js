...
client.once('ready', async () => {
  ...
  // Toutes les heures — chance aléatoire de déclencher un bonus de points dans #bataille
  cron.schedule('0 * * * *', async () => {
    try {
      await require('./bot/src/systems/faction').maybeTriggerMultiplierEvent(client);
    } catch (err) {
      logger.error('Bot', 'Erreur déclenchement bonus bataille', err);
    }
  }, { timezone: 'Europe/Paris' });
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton() || interaction.isStringSelectMenu()) { await handleButton(interaction, client); return; }
    ...
    const reply = { content: '❌ Une erreur est survenue. Réessaie, ou contacte le staff si ça persiste.', ephemeral: true };
  ...
  for (const guild of client.guilds.cache.values()) {
    for (const [, channel] of guild.channels.cache) {
      ...
      for (const [, member] of channel.members) {
        ...
        const xpGain = VOICE_XP_INTERVAL_MIN * VOICE_XP_PER_MIN;
        try {
          const updated = await User.findOneAndUpdate(
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (oldState.channelId && !newState.channelId) {
    ...
    try {
      const xpGained = minutes * VOICE_XP_PER_MIN; // déjà réellement crédité via le cron pendant la session
...
client.on('guildMemberAdd', async member => {
  try {
    ...
    // 1. Message de bienvenue (configurable depuis le dashboard)
    ...
    if (welcomeChannelId) {
      ...
      if (ch) {
        ...
        if (cfg?.welcomeEnabled && (cfg?.welcomeTitle || cfg?.welcomeDesc)) {
          // Embed personnalisé depuis le dashboard
          const { EmbedBuilder } = require('discord.js');
          const color = cfg.welcomeColor ? parseInt(cfg.welcomeColor.replace('#',''), 16) : 0x2ecc71;
          const embed = new EmbedBuilder().setColor(color);
          if (cfg.welcomeTitle)  embed.setTitle(fill(cfg.welcomeTitle));
          if (cfg.welcomeDesc)   embed.setDescription(fill(cfg.welcomeDesc));
          if (cfg.welcomeImage)  embed.setImage(cfg.welcomeImage);
          if (cfg.welcomeFooter) embed.setFooter({ text: fill(cfg.welcomeFooter) });
          const content = cfg.welcomeMention ? `<@${member.id}>` : undefined;
          ch.send({ content, embeds: [embed] }).catch(() => {});
        } else {
