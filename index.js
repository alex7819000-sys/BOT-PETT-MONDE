...
client.once('ready', async () => {
  ...
  // ── Annonce de mise à jour — poste le changelog si nouvelle version détectée ──
  require('./bot/src/systems/updateAnnounce').announceUpdatesIfNeeded(client).catch((err) => {
    logger.error('Bot', 'Erreur annonce mise à jour', err);
  });
  ...
  // Tous les dimanches à 00h00 — reset COMPLET : weekXp, totalXp, xp et level de
  // tout le monde repartent à 0, + retrait des rôles de niveau et rôles hebdo.
  cron.schedule('0 0 * * 0', async () => {
    try {
      ...
      for (const guild of client.guilds.cache.values()) {
        ...
        // ── King of Hera — trophée hebdo unique, calculé AVANT le reset ci-dessous ──
        // (sinon il n'y a plus aucun weekXp à comparer). Retiré à l'ancien détenteur,
        // donné au membre avec le plus d'XP cette semaine — aucun gagnant si personne
        // n'a gagné d'XP (le rôle est alors juste retiré à tout le monde).
        if (cfg?.kingOfHeraRoleId) {
          try {
            const role = guild.roles.cache.get(cfg.kingOfHeraRoleId);
            if (role) {
              const top = await User.findOne({ guildId: guild.id, weekXp: { $gt: 0 } }).sort({ weekXp: -1 }).lean();
              const members = await guild.members.fetch();
              for (const [, member] of members) {
                if (member.roles.cache.has(role.id) && (!top || member.id !== top.userId)) {
                  await member.roles.remove(role.id).catch(() => {});
                }
              }
              if (top) {
                const winner = members.get(top.userId);
                if (winner && !winner.roles.cache.has(role.id)) {
                  await winner.roles.add(role.id).catch(() => {});
                }
              }
            }
          } catch (err) {
            logger.error('Bot', 'Erreur attribution King of Hera', err);
          }
        }

        await User.updateMany({ guildId: guild.id }, { weekXp: 0 });

        // ── Reset complet XP totale + niveau chaque semaine ──────────────────
        // Remise à zéro de totalXp, xp (XP dans le niveau en cours) et level pour
        // TOUS les membres — en plus du reset weekXp ci-dessus. Contrairement au
        // weekXp qui ne suivait qu'un classement hebdo, la progression de niveau
        // était jusqu'ici permanente ; elle repart maintenant de zéro chaque
        // dimanche pour tout le serveur.
        await User.updateMany({ guildId: guild.id }, { totalXp: 0, xp: 0, level: 0 });

        // Retirer les rôles de niveau (paliers /setup levelrole) à tout le monde,
        // puisque tout le monde repart à niveau 0.
        if (cfg?.levelRoles?.length) {
          const levelRoleIds = cfg.levelRoles.map(lr => lr.roleId);
          try {
            const members = await guild.members.fetch();
            for (const [, member] of members) {
      ...
      logger.info('Bot', 'Reset XP hebdomadaire + XP totale/niveau + rôles effectué');
    } catch (err) {
...
client.on('messageCreate', async message => {
  ...
  try {
    ...
    // Activité par salon — compteur léger, pour /stats salons (quels salons sont réellement utilisés)
    require('./bot/src/db/models/ChannelActivity').updateOne(
      { guildId: message.guild.id, channelId: message.channel.id },
      { $inc: { messageCount: 1 }, $set: { lastMessageAt: new Date() } },
      { upsert: true }
    ).catch(() => {});

    // Chat IA — répond si on mentionne le bot ou dit son nom déclencheur (ne bloque
    // jamais le reste du traitement du message : XP, counting, etc. continuent normalement)
    require('./bot/src/systems/aiChat').handleAIChat(message, client).catch(() => {});
    ...
    if (newLevel > (updated.level || 0)) {
      ...
      const { applyLevelRoles } = require('./bot/src/systems/roleRewards');
      if (message.member) await applyLevelRoles(message.member, cfg, newLevel);
    }

    // ── Rôles hebdo (weekXp) — mis à jour à chaque message ──────────────
    if (!cfg) return; // cfg déjà chargé plus haut, sinon skip
    {
      const { applyWeeklyRoles } = require('./bot/src/systems/roleRewards');
      if (message.member) await applyWeeklyRoles(message.member, cfg, updated.weekXp || 0, message);
    }
    ...
    for (const [, channel] of guild.channels.cache) {
      ...
      for (const [, member] of channel.members) {
        ...
        try {
          ...
          const updated = await User.findOneAndUpdate(
            { userId: member.id, guildId: guild.id },
            { $inc: { xp: xpGain, totalXp: xpGain, weekXp: xpGain, vocalMinutes: VOICE_XP_INTERVAL_MIN, vocalMinutesToday: VOICE_XP_INTERVAL_MIN } },
            { upsert: true, new: true }
          );

          const cfg = await require('./bot/src/db/models/Config').findOne({ guildId: guild.id }).lean().catch(() => null);
          ...
          if (newLevel > (updated.level || 0)) {
            ...
            // Même logique de rôles que le texte — avant, seul le texte les donnait,
            // donc un membre actif uniquement en vocal montait de niveau sans jamais
            // recevoir son rôle. Corrigé (voir systems/roleRewards.js).
            const { applyLevelRoles } = require('./bot/src/systems/roleRewards');
            await applyLevelRoles(member, cfg, newLevel);
          }

          // Rôles hebdo — même correction : le vocal fait maintenant gagner du weekXp
          // (ligne $inc ci-dessus) et met à jour le rôle hebdo, comme le texte.
          const { applyWeeklyRoles } = require('./bot/src/systems/roleRewards');
          await applyWeeklyRoles(member, cfg, updated.weekXp || 0, fakeMessage);
...
const chatReviveCooldowns = new Map(); // `${guildId}` → timestamp dernier ping Chat Revive (anti-spam, 1 ping max toutes les 5 min)
client.on('guildMemberAdd', async member => {
  try {
    ...
    // 4. Chat Revive — ping le rôle + la personne qui vient d'arriver, dans le salon configuré
    // (anti-spam : 1 ping max toutes les 5 minutes par serveur, même si plusieurs membres rejoignent d'affilée)
    if (cfg?.chatReviveRoleId && cfg?.chatReviveChannelId) {
      const revLast = chatReviveCooldowns.get(member.guild.id);
      if (!revLast || Date.now() - revLast > 5 * 60_000) {
        const reviveCh = member.guild.channels.cache.get(cfg.chatReviveChannelId);
        if (reviveCh) {
          chatReviveCooldowns.set(member.guild.id, Date.now());
          reviveCh.send({ content: `<@&${cfg.chatReviveRoleId}> 🔔 <@${member.id}> vient d'arriver, venez lui dire coucou !` }).catch(() => {});

          // 4b. Le chat IA relance ensuite spontanément la conversation avec le
          // nouveau membre (léger délai pour ne pas arriver en même temps que le
          // ping) — silencieux si la clé Gemini n'est pas configurée ou le quota atteint.
          setTimeout(() => {
            require('./bot/src/systems/aiChat').greetNewMember(member, reviveCh).catch(() => {});
          }, 3000);
        }
      }
    }
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  try {
    ...
    if (!wasBooster && isBooster) {
      ...
      // Re-fetch le membre pour être sûr d'avoir des données fraîches (avatar inclus) —
      // le membre passé par l'event peut être une version en cache incomplète/périmée.
      const freshMember = await newMember.guild.members.fetch({ user: newMember.id, force: true }).catch(() => newMember);
      let avatarUrl = null;
      try { avatarUrl = freshMember.displayAvatarURL({ size: 256, extension: 'png' }); } catch { /* on gère l'absence juste en-dessous */ }
      if (!avatarUrl) {
        try { avatarUrl = newMember.user?.displayAvatarURL({ size: 256, extension: 'png' }); } catch { /* dernier recours échoué, pas grave */ }
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `**${freshMember.displayName}** vient de booster **${newMember.guild.name}** ! 🚀\n\n` +
          `${perks.join('\n')}\n\n` +
