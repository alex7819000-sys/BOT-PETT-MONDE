  ...
  try {
    ...
    if (cfg?.welcomeCardEnabled && cfg?.welcomeCardChannelId) {
      ...
      if (cardCh) {
        ...
        const embed = new EmbedBuilder()
          .setColor(color)
          .setTitle(cfg.welcomeTitle ? fill(cfg.welcomeTitle) : `🎉 Bienvenue sur ${member.guild.name} !`)
          .setDescription(
            (cfg.welcomeDesc ? fill(cfg.welcomeDesc) : `Hey <@${member.id}>, on est ravis de t'accueillir parmi nous ! 🦎\n\nTu es le **${memberCount}e** membre du serveur.\nPrends le temps de lire les règles et profite bien de la communauté !`)
          )
          .addFields(
            { name: '👤 Membre', value: `${member.user.username} ( ${member.id} )`, inline: true },
            { name: '👥 Membres', value: `${memberCount} membres`, inline: true },
          )
        ...
        cardCh.send({ content: cardContent, embeds: [embed] }).catch(() => {});
      }
    }
