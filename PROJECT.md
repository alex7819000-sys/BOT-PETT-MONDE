...
> Dernière mise à jour : v90 (août 2026)

---

## Version actuelle : v90

Base : v89 → v88 → v87 → v86 → v85 → v84 → v83 → v82 → v81 → v80 → v79 → v78 → v77 → v76 → v75 → v74 → v73 → v72 → v71 → v70 → v69 → v68 → v63 → v62 → v61 → v60 → v59 → v56 (corrigé depuis v55 → v54 → v43 → v20)
...
| 🤖 Chat IA (répond aux membres) | `/notif ia nom:sayen` (nécessite `GEMINI_API_KEY` dans les variables d'environnement Render — gratuit, voir section dédiée) |
...
- **100% gratuit** — utilise l'API Google Gemini (modèle `gemini-2.5-flash` par défaut, changeable via la variable d'env `GEMINI_MODEL`), qui offre un vrai palier gratuit sans carte bancaire (contrairement à OpenAI/Anthropic, payants dès le premier usage)
...
**Limites du plan gratuit Google (à vérifier soi-même, ça évolue vite)** : au moment de la construction (août 2026), de l'ordre de plusieurs centaines à ~1500 requêtes/jour selon le modèle exact utilisé — Google fait évoluer ces chiffres fréquemment. Sur le plan gratuit, Google peut utiliser les échanges pour améliorer ses modèles (pas le cas sur un plan payant) — accepté en connaissance de cause pour ce cas d'usage.
