...
> Dernière mise à jour : v100 (septembre 2026)

---

## Version actuelle : v100

Base : v99 → v98 → v97 → v96 → v95 → v94 → v93 → v92 → v91 → v90 → v89 → v88 → v87 → v86 → v85 → v84 → v83 → v82 → v81 → v80 → v79 → v78 → v77 → v76 → v75 → v74 → v73 → v72 → v71 → v70 → v69 → v68 → v63 → v62 → v61 → v60 → v59 → v56 (corrigé depuis v55 → v54 → v43 → v20)
...
### ⚠️ Piège connu — mise à jour du code sur GitHub
Le code n'est pas géré via `git push` mais via l'interface web GitHub ("Add files via upload"). Vécu plusieurs fois dans ce projet : le nouveau code n'arrive jamais vraiment sur GitHub malgré un upload "réussi" en apparence, et Render redéploie alors l'ancien code sans qu'aucune erreur ne le signale (le déploiement réussit, juste avec le mauvais contenu — repérable seulement en comparant les logs/versions avant-après, ou en allant vérifier directement qu'un fichier précis existe sur le repo).

**Cause identifiée** : le zip livré doit être **extrait sur l'ordinateur avant l'upload** (clic droit → "Extraire tout"). Envoyer le fichier `.zip` tel quel, ou mal extraire des dossiers imbriqués, fait que GitHub n'enregistre pas correctement l'arborescence complète.

**Réflexe à avoir après chaque déploiement** : vérifier dans les logs Render que la ligne `[INFO][UpdateAnnounce] vXX annoncé` affiche bien la **dernière version livrée** (voir `config/changelog.js` → `CURRENT_VERSION`) — si elle affiche une version plus ancienne, c'est que l'upload n'a pas pris, il faut recommencer en extrayant bien le zip d'abord. *(Ce log n'apparaît que si `/notif maj` a été configuré — sinon, vérifier directement sur GitHub qu'un fichier récent existe, ex: `bot/src/systems/aiChat.js`.)*
...
- **Nouvelles tentatives automatiques (v96-v97)** : si Google répond "surchargé" (503/429) ou met trop de temps, jusqu'à **5 tentatives** avec pause croissante (2s/4s/6s/8s) avant d'abandonner — accepté sciemment que ça puisse prendre jusqu'à ~2 minutes dans le pire cas, priorité donnée à la réussite plutôt qu'à la rapidité (demandé explicitement). L'indicateur "en train d'écrire" est relancé toutes les 8s pendant l'attente pour ne pas donner l'impression d'un plantage. Ne réessaie jamais sur une vraie erreur de config (404, 400) où ça ne changerait rien
- **Anti-troncature (v98)** : Gemini coupait parfois sa réponse en plein milieu d'un mot (limite de longueur atteinte avant la fin de la phrase). Limite remontée à 500 tokens + fonction `cleanTruncatedReply` qui coupe proprement à la dernière phrase complète plutôt que de laisser un mot à moitié écrit
...
| v95 | Suppression du système "feur" (easter egg quoi→feur/comment→tateur, demandé directement) : fichier `systems/feur.js`, appel dans `index.js`, champ `feurChannelId` et ses 2 entrées de configuration (`/setup salon`, dashboard) retirés proprement. Chat IA : nouveau signalement d'échecs intermittents ("j'ai eu un souci pour répondre") constatés en usage réel — diagnostic en cours, logs Render demandés pour identifier la cause exacte avant de corriger à l'aveugle (peut être différent du problème de modèle déjà réglé en v91 : quota, limite/minute, ou timeout réseau) |
| v96 | Diagnostic du bug chat IA confirmé par les logs : deux erreurs différentes du fix de v91, toutes deux liées à une **surcharge côté serveurs Google** sur `gemini-3.6-flash` (`503 UNAVAILABLE — high demand`) et un dépassement du délai de 15s dans un cas plus lent. Rien à voir avec notre code, mais rendu plus robuste : ajout d'un système de **nouvelle tentative automatique** (jusqu'à 3 essais, avec 1s puis 2s de pause entre chaque) qui ne se déclenche que sur les erreurs réellement temporaires (503, 429, timeout) — pas sur une vraie erreur de configuration (404 mauvais modèle, 400 requête invalide) où réessayer ne changerait rien. Délai réseau passé de 15s à 20s de marge. Message d'erreur final différencié : précise "les serveurs de Google sont surchargés" quand c'est réellement le cas, plutôt qu'un message générique qui laissait croire à un bug du bot |
| v97 | Renforcement du retry du chat IA à la demande ("au pire mets un délai, je veux vraiment éviter ça") : 3→**5 tentatives** max, pause croissante 2s/4s/6s/8s (au lieu de 1s/2s), délai réseau par tentative 20s→**25s**. Dans le pire des cas ça peut prendre jusqu'à ~2 minutes avant d'abandonner — accepté sciemment, priorité donnée à la réussite plutôt qu'à la rapidité, sur demande explicite. Ajout d'un rafraîchissement de l'indicateur "en train d'écrire" toutes les 8s pendant l'attente (Discord l'arrête tout seul après ~10s), pour que ça ne donne pas l'impression que le bot a planté pendant les tentatives longues |
| v98 | Signalé avec captures à l'appui : les réponses du chat IA se coupaient parfois en plein milieu d'un mot ("Ça dit quoi aujourd'", "je m'é", "la commu va k"). Cause : la limite de longueur imposée à Gemini (300 "tokens") était atteinte avant la fin de la phrase, et rien ne nettoyait le résultat avant de l'envoyer. Corrigé en 2 temps : limite remontée à 500 tokens (réduit la fréquence du problème), et ajout d'une fonction `cleanTruncatedReply` qui retrouve la fin de la dernière phrase complète et coupe proprement là plutôt que de laisser un mot à moitié écrit (ou ajoute "…" en dernier recours si aucune phrase n'est complète du tout). Bug trouvé et corrigé pendant l'implémentation : la 1ère version du nettoyage considérait à tort qu'une réponse terminée par une apostrophe était "complète" — dans les faits, un mot français coupé en plein milieu ("aujourd'hui" → "aujourd'") se termine aussi par une apostrophe, donc ce test donnait un faux positif systématique sur ce cas précis. Testé avec les 3 exemples exacts remontés avant livraison |
| v99 | Suppression du système "sass" (demandé directement, capture à l'appui d'un exemple concret : "ratio + t'as répondu à un bot..."). C'était un système à part, distinct du "feur" retiré en v95 et distinct du chat IA — réagissait aux insultes et au "brainrot" (six-seven, sigma, gyat...) avec des phrases pré-écrites, tirées au hasard dans des listes fixes. Devenu redondant maintenant que le vrai chat IA existe. Retiré proprement : fichier `systems/sass.js`, appel dans `index.js`, champ `sassEnabled` et sa case à cocher dans le dashboard (page "Général") |
| v100 | Suite à un handoff écrit dans une autre conversation (refonte du contenu du serveur HERA — règlement staff, structure de salons, annonces), vérification du créateur d'embeds du dashboard (`/embeds`), identifié comme point bloquant. **2 bugs réels trouvés** : **(1)** un bouton de style "Lien URL" sans URL renseignée faisait planter l'envoi de **tout le message** (Discord.js refuse un bouton Link sans URL) — même l'embed ne partait plus, sans erreur visible côté utilisateur ; corrigé en filtrant silencieusement le bouton incomplet plutôt que de tout annuler. **(2)** les boutons "de couleur" (bleu/gris/vert/rouge) créaient un `customId` unique (`embed_btn_X_timestamp`) pour lequel **aucun gestionnaire n'a jamais existé nulle part dans le bot** — décoratifs, cliquer dessus affichait juste "Cette interaction a échoué". Remplacé par un vrai système : chaque bouton non-lien choisit maintenant une **action réelle déjà existante** dans le bot (Ouvrir un ticket, Faire un signalement, Candidater au staff, Demande de partenariat), reliée aux handlers réels (`ticket:open:support`, `sigrep:start`, `staff:candidater`, `partner:demande`). Nouveau champ `action` sur le schéma `SavedEmbed.buttons`, avec repli automatique sur "lien" pour les embeds sauvegardés avant ce changement |
| v101 | Ajout de "Chat Revive" (demandé directement, suite à une discussion sur 2 salons `sanction`/`sanctions` mal nommés qui a mené à clarifier tout le système de notifications) : à chaque arrivée d'un nouveau membre, le bot ping un rôle dédié + mentionne directement la personne, dans un salon choisi, pour inciter la communauté à venir l'accueillir. Implémenté dans le handler `guildMemberAdd` déjà existant dans `index.js`. Config exposée de 2 façons équivalentes : commande dédiée tout-en-un `/setup chatrevive salon: role: minutes:` (chaque champ optionnel et indépendant ; sans argument, affiche la config actuelle), et séparément via les commandes génériques déjà existantes `/setup role` (nouveau type `chatReviveRoleId`) et `/setup salon` (nouveau type `chatReviveChannelId`) — les deux écrivent dans les mêmes champs Mongo, donc interchangeables selon la préférence. Nouveaux champs `chatReviveChannelId`/`chatReviveRoleId`/`chatReviveMinutes` ajoutés au schéma `Config.js` (nécessaire : le schéma Mongoose est strict, un champ non déclaré explicitement est silencieusement ignoré à l'écriture). Anti-spam ajouté d'entrée de jeu suite à une question directe ("ça fait quoi si plusieurs personnes rejoignent d'un coup") : cooldown en mémoire de 5 minutes par serveur, un seul ping part même si plusieurs membres arrivent d'affilée. **Point non résolu, assumé explicitement** : le champ `minutes` (délai d'inactivité) est enregistré en base mais n'a aucun effet — c'est un nom déjà présent dans la commande fantôme `/setup chatrevive` découverte en v93, gardé tel quel pour ne pas perdre la donnée si quelqu'un l'avait déjà réglée, mais la vraie fonctionnalité de ping-sur-inactivité reste à construire si le besoin est confirmé (même logique que `dmblast`/`liveboard`/etc., laissés de côté en v93 pour la même raison) |
| v102 | Suite au retour direct sur le rendu de Chat Revive en conditions réelles (capture à l'appui), ajout d'un message spontané généré par le chat IA (`systems/aiChat.js`) juste après le ping, pour engager la conversation avec le nouveau membre sans que ça ressemble à un message d'accueil automatique. Nouvelle fonction exportée `greetNewMember(member, channel)` : prompt système dédié (ton "membre du serveur qui accueille quelqu'un", distinct du prompt de conversation normale), réutilise le même quota journalier et la même limite 15/min que le reste du chat IA (pas de double-comptage, juste un appel de plus dans le même compteur), déclenché avec 3s de délai après le ping pour ne pas arriver en même temps. L'échange est ajouté à la mémoire de conversation du salon (`pushToHistory`) donc si le nouveau membre répond, le chat IA se souvient déjà de son propre message d'accueil. Repli silencieux si `GEMINI_API_KEY` absente ou quota épuisé — reste un bonus, ne bloque jamais le ping ni le reste de l'arrivée du membre. Point de vigilance repris directement du prompt système existant du chat IA et volontairement conservé à l'identique : le bot ne doit jamais prétendre être humain s'il lui est demandé directement s'il est un bot |
| v103 | Suite à une question sur des rôles de niveau apparemment mal attribués, audit complet du système de rôles de niveau (`roleRewards.js`, `levelUp.js`, `/setup levelrole`, `/xp/level-rewards` dashboard) : logique vérifiée saine (à chaque niveau franchi, tous les paliers configurés sont recontrôlés et les rôles manquants ajoutés automatiquement — auto-réparant à chaque nouveau niveau même si un cas précédent avait échoué). L'absence de champ "🎁 Tes rôles" sur l'embed niveau 2 de Mimo s'explique simplement : paliers configurés à 1/5/10/20/30/40/50, donc rien de nouveau à débloquer entre 1 et 2. Suite à cette clarification, changement de direction demandé directement : suppression des paliers hebdo à seuils multiples (`weeklyLevelRoles`, supprimés côté configuration) au profit d'un **rôle hebdomadaire unique "King of Hera"** — chaque dimanche à minuit (même cron que le reset d'XP hebdo, calculé juste avant la remise à zéro sinon plus rien à comparer), le membre avec le plus d'XP de la semaine reçoit le rôle, retiré à l'ancien détenteur ; aucun gagnant désigné si personne n'a gagné d'XP cette semaine-là. Nouveau champ `kingOfHeraRoleId` sur `Config.js`, configurable via `/setup role` (type "👑 King of Hera") — distinct du rôle `kingRoleId` déjà existant (rôle King permanent, sans rapport avec l'XP) |
| v104 | Complément direct du système de rôles de niveau suite à la clarification en v103 ("attribuer les rôles au niveau/xp, t'as capté ou pas") : ajout de `/setup syncroles`, une commande de rattrapage qui parcourt tous les membres actuels du serveur, recalcule leur niveau depuis leur `totalXp` (même formule que partout ailleurs dans le bot), met à jour `User.level` si besoin, et réapplique `applyLevelRoles` pour chacun — corrige donc rétroactivement tout membre qui aurait un niveau en base sans avoir le rôle correspondant (rôle retiré par erreur, palier de rôle ajouté après que le membre ait déjà atteint ce niveau, etc.), cas que la logique existante ne couvrait pas puisqu'elle ne se déclenche normalement qu'au moment précis où un niveau est franchi |
